...
# ============================================================
# 3. Transaction Monitoring Page (unified style)
# ============================================================
class TransactionMonitoringPage:
    """
    AML / CFT – Transaction Monitoring dashboard, aligned with unified SACCO Pro shell.

    • Uses legacy AML transaction analytics (sacco_core.aml.transactions.*)
    ...
    def _check_access(self) -> bool:
        ...
        has_access = self.rbac_manager.check_page_access(
            page_id="3_💳_Transaction_Monitoring.py",
        ...
        if not has_access:
            ...
            try:
                self.audit_logger.log_action(
                    ...
                    object_type="page",
                    object_id="Transaction_Monitoring",
                )
            except Exception:
                pass
            return False
    ...
    def run(self):
        ...
        with tabs[0]:
            ...
            if feed.empty:
                ...
                with c3:
                    st.metric(
                        "Max Amt",
                        f"{feed['amount'].abs().max():,.0f}"
                        if "amount" in feed
                        else "—",
                    )
                with c4:
                    by_ch = (
                        feed["channel"].value_counts().head(1).index[0]
                        if "channel" in feed and not feed["channel"].empty
                        else "—"
                    )
        ...
        with tabs[2]:
            ...
            window_days = c1.number_input(
                "Window (days)", min_value=3, max_value=30, step=1, value=7
            )
            unit_kes = c2.number_input(
                "Unit (KES)",
                min_value=10_000,
                max_value=5_000_000,
                step=10_000,
            ...
            near_ratio = c3.slider(
                "Near-unit ratio", min_value=0.1, max_value=1.0, value=0.8, step=0.05
            )
            ...
            if stairs.empty:
                st.info(
                    "No structuring staircase patterns detected for the selected filters."
                )
            else:
                ...
                member_list = (
                    stairs["member_id"].astype(str).unique().tolist()
                    if "member_id" in stairs
                    else []
                )
                if member_list:
                    sel = st.selectbox("Member", member_list)
                    seg = stairs[
                        stairs["member_id"].astype(str) == str(sel)
                    ].sort_values("day")
                    if not seg.empty:
                        fig = go.Figure()
                        fig.add_trace(
                            go.Scatter(
                                ...
                                y=seg["cash_in"],
                                mode="lines+markers",
                                name="Rolling Cash-in",
                            )
                        )
                        if "near_flag" in seg.columns:
                            near_seg = seg[seg["near_flag"] is True]
                            fig.add_trace(
                                go.Scatter(
                                    x=near_seg["day"],
                                    y=near_seg["cash_in"],
...
if __name__ == "__main__":
    page = TransactionMonitoringPage()
    page.run()
