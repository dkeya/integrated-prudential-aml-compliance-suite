...
# Make sure Python can see the project root so `core.*` imports work
...
from core.analytics.dq_scans import DataQualityScanner
from core.sidebar import render_sidebar

# ❌ REMOVE per-page set_page_config – app.py already handles global config
# st.set_page_config(
...
# Render consistent sidebar and styling via shared core.sidebar
render_sidebar()
...
class DataQualityScansPage:
    def __init__(self):
        ...
        # ✅ Use the shared singleton audit_logger from core.audit
    ...
    def _check_access(self):
        ...
        # If RBAC manager doesn't implement check_page_access, fail open (or adjust as needed)
        has_access = True
        if hasattr(self.rbac_manager, "check_page_access"):
        ...
        try:
            ...
            print(
                f"[AUDIT] data_quality_scans_page accessed by "
    ...
    def render_threat_intelligence(self):
        ...
        with col2:
            ...
            fig.add_trace(go.Scatter(
                x=list(predictions['future_dates']) + list(predictions['future_dates'][::-1]),
    ...
    def _execute_emergency_protocol(self, protocol):
        ...
        # ✅ Log safely depending on what AuditLogger supports
        try:
            # If core.audit.AuditLogger has log_action
            ...
            # Fallback to log_data_access
            try:
                self.audit_logger.log_data_access(
                    st.session_state.user,
                    st.session_state.role,
                    f"execute_emergency_protocol:{protocol}"
                )
            except Exception:
                # Final fallback: no-op
                pass
    
    # Preserved helper methods (you can fill later as needed)
