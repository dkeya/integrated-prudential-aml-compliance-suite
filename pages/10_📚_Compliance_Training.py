...
try:
    ...
    log_page_access = None  # optional, legacy only
...
# Render unified SACCO Pro sidebar (no page config here – handled in app.py)
render_sidebar()


# ============================================================
# 3. Compliance Training Page – unified wrapper
# ============================================================
class ComplianceTrainingPage:
    """
    AML / CFT — Training & Awareness

    Runs the existing AML training engine inside the unified SACCO Pro shell:
    - RBAC via core.rbac
    - Unified audit via core.audit.audit_logger
    - Optional legacy audit via sacco_core.audit
    """
    ...
    def _check_access(self) -> bool:
        ...
        has_access = self.rbac_manager.check_page_access(
            page_id="10_📚_Compliance_Training.py",
    ...
    # ---------------- Main render ----------------
    def run(self):
        # Light seed & housekeeping (same as original but inside unified shell)
        ...
        # 🔹 Precompute a default reminder badge (non-intrusive)
        ...
        try:
            ...
            if not _due_df.empty and "due_status" in _due_df.columns:
                _badge = len(
                    _due_df[_due_df["due_status"].isin(["Expiring Soon", "Expired"])]
                )
        except Exception:
            _badge = 0
        ...
        with tabs[0]:
                ...
                with st.expander("➕ Add / Update Course"):
                    ...
                    if mode == "Add new":
                        ...
                        cid = st.selectbox(
                            ...
                            [None]
                            + (
                                cat["course_id"].astype(int).tolist()
                                if not cat.empty
                                else []
                            ),
                    ...
                    category = st.text_input(
                        "Category",
                        value="Compliance",
                        key="course_category_input",
                    )
                    provider = st.text_input(
                        "Provider", value="Internal", key="course_provider_input"
                    )
                    hours = st.number_input(
                        ...
                        value=1.0,
                        step=0.5,
                        key="course_hours_input",
                    )
                    mode_str = st.selectbox(
                        "Delivery",
                        ["eLearning", "Workshop", "Webinar"],
                        key="course_delivery_select",
                    )
                    active = st.checkbox(
                        "Active", value=True, key="course_active_checkbox"
                    )
                    if st.button(
                        "💾 Save course",
                        ...
                        new_id = upsert_course(
                            cid, title, category, provider, hours, mode_str, active
                        )
            ...
            with c2:
                ...
                status = filt_c1.selectbox(
                    "Status",
                    ["All", "Assigned", "In-Progress", "Completed", "Expired"],
                    key="assign_status_select",
                )
                branch_opt = ["All"] + st.session_state.get("flt_branch_opts", [])[1:]
                branch = filt_c2.selectbox(
                    "Branch (if available)",
                    branch_opt,
                    key="assign_branch_select",
                )
                q_member = filt_c3.text_input(
                    "Search member (name contains)",
                    key="assign_search_member_input",
                )

                df = list_assignments(
                    status=status, branch=branch, search_member=q_member
                )
                ...
                if (
                    not df.empty
                    and "attachment_path" in df.columns
                    and df["attachment_path"].notna().any()
                ):
                    with st.expander("🔎 View Evidence (Assignments)"):
                        avail = df[df["attachment_path"].notna()][
                            ["assignment_id", "attachment_path"]
                        ]
                        ...
                        path_str = avail[
                            avail["assignment_id"] == sel_aid
                        ]["attachment_path"].iloc[0]
                        ...
                        try:
                            ...
                            st.info(
                                "Could not open file (path may be outside app)."
                            )

                # Assign a course
                with st.expander("➕ Assign Course"):
                    member_id = st.text_input(
                        "Member ID", key="assign_member_id_input"
                    )
                    ...
                    due_on = st.date_input(
                        "Due on",
                        value=date.today() + timedelta(days=14),
                        key="assign_due_on_date",
                    )
                    notes = st.text_area(
                        "Notes",
                        placeholder="Optional assignment note…",
                        key="assign_notes_area",
                    )
                    if st.button(
                        "📌 Assign",
                        use_container_width=True,
                        key="assign_course_btn",
                    ):
                        if member_id.strip():
                            aid = assign_course(
                                member_id.strip(),
                ...
                with st.expander("✅ Mark Completed"):
                    if df.empty:
                        ...
                        open_rows = df[
                            df["status"].isin(["Assigned", "In-Progress"])
                        ]
                        if open_rows.empty:
                            ...
                            sel = st.selectbox(
                                "Assignment",
                                open_rows["assignment_id"].tolist(),
                                key="complete_assignment_select",
                            )
                            score = st.number_input(
                                ...
                                value=100.0,
                                step=0.5,
                                key="complete_score_input",
                            )
                            up = st.file_uploader(
                                "Upload evidence (optional)",
                                type=["pdf", "png", "jpg", "jpeg"],
                                key="complete_evidence_uploader",
                            )
                            completed_on = st.date_input(
                                "Completed on",
                                value=date.today(),
                                key="complete_completed_on_date",
                            )
                            if st.button(
                                "✔️ Record Completion",
                                use_container_width=True,
                                key="complete_record_btn",
                            ):
        ...
        with tabs[1]:
            ...
            with c1:
                ...
                status = filt1.selectbox(
                    "Status",
                    ["All", "Valid", "Expiring", "Expired", "Revoked"],
                    key="cert_status_select",
                )
                branch_opt = ["All"] + st.session_state.get(
                    "flt_branch_opts", []
                )[1:]
                branch = filt2.selectbox(
                    "Branch (if available)",
                    branch_opt,
                    key="cert_branch_select",
                )
                q_member = filt3.text_input(
                    "Search member (name contains)",
                    key="cert_search_member_input",
                )
                certs = list_certifications(
                    branch=branch, status=status, search_member=q_member
                )
                st.dataframe(certs, use_container_width=True, height=380)
                st.download_button(
                    "⬇️ Export CSV",
                    certs.to_csv(index=False).encode("utf-8"),
                    "certifications.csv",
                    mime="text/csv",
                    use_container_width=True,
                    key="cert_export_btn",
                ...
                if (
                    not certs.empty
                    and "certificate_path" in certs.columns
                    and certs["certificate_path"].notna().any()
                ):
                    with st.expander("🔎 View Certificate"):
                        availc = certs[certs["certificate_path"].notna()][
                            ["cert_id", "certificate_path"]
                        ]
                        ...
                        cpath_str = availc[
                            availc["cert_id"] == sel_cid
                        ]["certificate_path"].iloc[0]
            ...
            with c2:
                ...
                within = st.slider(
                    ...
                    min_value=7,
                    max_value=180,
                    value=_DEFAULT_REMINDER_DAYS,
                    step=7,
                    key="cert_within_slider",
                )
                ...
                cname = st.text_input(
                    "Certification Name", key="cert_name_input"
                )
                cprov = st.text_input(
                    "Provider",
                    value="Internal/External",
                    key="cert_provider_input",
                )
                issued = st.date_input(
                    "Issued on", value=date.today(), key="cert_issued_date"
                )
                expires = st.date_input(
                    "Expires on",
                    value=date.today() + timedelta(days=365),
                    key="cert_expires_date",
                )
                ...
                if st.button(
                    ...
                    if cmid.strip() and cname.strip():
                        ...
                        st.warning(
                            "Member ID and Certification Name are required."
                        )
        ...
        with tabs[2]:
            ...
            cat = s2.text_input(
                "Category filter (optional)", key="kb_category_filter"
            )
            tag = s3.text_input(
                "Tag contains (optional)", key="kb_tag_filter"
            )
            res = search_kb(
                q=q or None, category=(cat or None), tag=(tag or None)
            )
            ...
            with st.expander("➕ Add Article"):
                ...
                kb = st.text_area(
                    "Body (short guide / notes)", key="kb_add_body"
                )
                kc = st.text_input(
                    "Category", value="Guides", key="kb_add_category"
                )
                ku = st.text_input("URL (optional)", key="kb_add_url")
                ktags = st.text_input(
                    "Tags (comma-separated)", key="kb_add_tags"
                )
                if st.button(
                    "📌 Save Article",
                    use_container_width=True,
        ...
        with tabs[3]:
            ...
            if tr.empty:
                ...
                fig = px.line(
                    ...
                    y="completions",
                    markers=True,
                    title=f"Completions by {by.title()}",
                )
                ...
                if "avg_score" in tr.columns:
                    fig2 = px.bar(
                        ...
                        x="period",
                        y="avg_score",
                        title=f"Average Score by {by.title()}",
                    )
