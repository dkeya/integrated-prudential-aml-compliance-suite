...
# 🔽 LOCAL / PROJECT IMPORTS (unified core.* structure)
...
# Optional: page-level config (ok in multipage apps when run standalone)
st.set_page_config(
...
# ==============================================
# SIMPLE GOVERNANCE ANALYZER STUB
# (can be replaced by core.analytics.governance.GovernanceAnalyzer later)
# ==============================================
class GovernanceAnalyzer:
    """
    Lightweight stub to provide governance analytics data.
    Replace with a full implementation later if needed.
    """

    def calculate_compliance_score(self, context: dict) -> dict:
        # Simple static score for now
        return {"overall_score": 88.0}

    def get_compliance_trends(self) -> pd.DataFrame:
        # Sample trend data (aligned with fallback structure)
        months = pd.date_range(start="2023-07-01", end="2024-02-01", freq="ME")
    ...
    def monitor_policy_compliance(self, policies):
        # Placeholder – page only needs this callable
        return {}

    def track_audit_findings(self, findings):
        # Placeholder – page only needs this callable
        return {}

    def generate_regulatory_calendar(
        self, start_date: str, end_date: str
    ) -> pd.DataFrame:
        # Return empty so the page uses its existing fallback schedule
        return pd.DataFrame()
...
class GovernanceCompliancePage:
    ...
    def _check_access(self) -> bool:
        """Check RBAC access and log page access safely."""
        if not st.session_state.get("authenticated", False):
        ...
        if not has_access:
            ...
            try:
                self.audit_logger.log_action(
                    ...
                    object_type="page",
                    object_id="05_Governance_Compliance.py",
                )
            except Exception:
                pass
            return False

        # Log successful access (best effort, with fallbacks)
        try:
    ...
    def render_compliance_dashboard(self):
        ...
        # Get compliance score from analyzer (stubbed)
    ...
    def render_compliance_status(self):
        ...
        styled_df = compliance_data.style.applymap(color_status, subset=["Status"])
    ...
    def render_governance_framework(self):
        ...
        with col2:
            ...
            completion_rate = (
                meeting_data["Completed"].sum()
                / meeting_data["Action Items"].sum()
                * 100
            )
    ...
    def render_policy_management(self):
        ...
        # Placeholder – currently not used directly, but keeps API consistent
        _ = self.governance_analyzer.monitor_policy_compliance([])
    ...
    def render_audit_findings(self):
        ...
        # Placeholder – not used further but keeps API consistent
        _ = self.governance_analyzer.track_audit_findings([])
        ...
        with col3:
            completion_rate = (
                len(audit_data[audit_data["Status"] == "Completed"])
                / len(audit_data)
                * 100
            )
    ...
    def render_regulatory_reporting(self):
        ...
        reporting_calendar = self.governance_analyzer.generate_regulatory_calendar(
            start_date, end_date
        )
            ...
            events.append(
                {
                    ...
                    "Status": "Pending" if days_remaining > 0 else "Due",
                }
            )
