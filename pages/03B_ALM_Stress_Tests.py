...
import sys
import os
import time  # needed for sleep in recovery analysis
...
from core.audit import AuditLogger, audit_logger  # unified audit logger
from core.sidebar import render_sidebar

# ==============================================
# SIMPLE LOCAL STRESS TEST ENGINE (STUB)
# ==============================================
class ALMStressTester:
    """
    Lightweight ALM stress tester stub.
    Generates deterministic, illustrative stress results so the page works
    even without a full backend engine.
    """

    def run_comprehensive_stress_tests(self, scenarios: dict, scenario_groups: list):
        # Baseline metrics (ratios as decimals, amounts as absolute numbers)
        baseline = {
            "liquidity_ratio": 0.185,        # 18.5%
            "capital_adequacy": 0.221,       # 22.1%
            "net_interest_income": 60_000_000,  # KES
            "funding_gap": 0.0,
            "ecl_provision": 5_000_000,
            "roa": 0.018,                    # 1.8%
            "lcr": 1.25,                     # 125%
            "nsfr": 1.15,                    # 115%
            "status": "Pass",
            "description": "Baseline business-as-usual position"
        }

        results = {"baseline": baseline}

        # Helper to build a scenario
        def make_scenario(name, liq_mult, cap_mult, nii_mult, fail_threshold=0.10):
            liq = baseline["liquidity_ratio"] * liq_mult
            cap = baseline["capital_adequacy"] * cap_mult
            nii = baseline["net_interest_income"] * nii_mult

            status = "Pass"
            if liq < fail_threshold or cap < 0.15:
                status = "Fail"
            ...
            return {
                ...
                "funding_gap": baseline["funding_gap"] + (baseline["net_interest_income"] - nii) * 0.1,
                "ecl_provision": baseline["ecl_provision"] * (1.0 + (1 - nii_mult)),
                "roa": baseline["roa"] * nii_mult,
                "lcr": baseline["lcr"] * liq_mult,
                "nsfr": baseline["nsfr"] * cap_mult,
                "status": status,
                "description": f"{name.replace('_', ' ').title()} scenario"
            }

        # Build a few key scenarios based on sliders
        # Use severity derived from sliders (normalized where useful)
        mild_runoff = scenarios.get("mild_deposit_runoff", 0.10)     # 0–1
        severe_runoff = scenarios.get("severe_deposit_runoff", 0.25)
        extreme_runoff = scenarios.get("extreme_deposit_runoff", 0.40)

        # Convert runoff into simple multipliers
        def liq_multiplier(runoff):
            # very rough: higher runoff -> lower multiplier
            return max(0.05, 1.0 - runoff * 0.8)

        # Deposit run-off scenarios
        results["mild_deposit_runoff"] = make_scenario(
            "mild_deposit_runoff",
            liq_mult=liq_multiplier(mild_runoff),
            cap_mult=0.98,
            nii_mult=0.98,
        )
        results["severe_deposit_runoff"] = make_scenario(
            "severe_deposit_runoff",
            liq_mult=liq_multiplier(severe_runoff),
            cap_mult=0.95,
            nii_mult=0.95,
        )
        results["extreme_deposit_runoff"] = make_scenario(
            "extreme_deposit_runoff",
            liq_mult=liq_multiplier(extreme_runoff),
            cap_mult=0.90,
            nii_mult=0.90,
            fail_threshold=0.08,
        )

        # Rate shock scenarios (use BPS sliders)
        mild_rate = scenarios.get("mild_rate_shock_bps", 50)
        severe_rate = scenarios.get("severe_rate_shock_bps", 200)
        extreme_rate = scenarios.get("extreme_rate_shock_bps", 400)

        def rate_impact(bps):
            # 0–1000 bps mapped to 0–20% NII reduction
            return min(0.20, bps / 5000.0)

        results["mild_rate_shock"] = make_scenario(
            "mild_rate_shock",
            liq_mult=1.0,
            cap_mult=0.99,
            nii_mult=1.0 - rate_impact(mild_rate),
        )
        results["severe_rate_shock"] = make_scenario(
            "severe_rate_shock",
            liq_mult=0.97,
            cap_mult=0.97,
            nii_mult=1.0 - rate_impact(severe_rate),
        )
        results["extreme_rate_shock"] = make_scenario(
            "extreme_rate_shock",
            liq_mult=0.93,
            cap_mult=0.94,
            nii_mult=1.0 - rate_impact(extreme_rate),
        )

        # Credit deterioration scenario (use PD / LGD / collateral sliders)
        pd_severe = scenarios.get("severe_pd_increase", 0.15)
        lgd_inc = scenarios.get("lgd_increase", 0.20)
        collateral_haircut = scenarios.get("collateral_haircut", 0.10)

        credit_severity = min(0.5, pd_severe + lgd_inc + collateral_haircut)  # 0–0.5

        results["credit_quality_deterioration"] = make_scenario(
            "credit_quality_deterioration",
            liq_mult=1.0 - credit_severity * 0.4,
            cap_mult=1.0 - credit_severity * 0.5,
            nii_mult=1.0 - credit_severity * 0.3,
        )

        # Mark which groups we actually used (for future extension)
        results["_meta"] = {
...
class ALMStressTestPage:
    def __init__(self):
        ...
        self.audit_logger = audit_logger  # use shared audit logger
    ...
    def _check_access(self):
        ...
        # Log data access (wrapped in try just in case)
        try:
        ...
        with col_run2:
            if st.button(
                ...
                with st.spinner("🌊 Running stress tests across all scenarios..."):
                    ...
                    # Run stress tests via local tester
    ...
    def render_scenario_impact_analysis(self):
        ...
        for scenario_name, scenario_data in results.items():
            if scenario_name in ("baseline", "_meta"):
                continue
            ...
            for metric in key_metrics:
                ...
                if metric_key in scenario_data:
                    ...
                    # Convert to floats if they happen to be strings
                    if isinstance(baseline_value, str):
                        baseline_value = float(
                            baseline_value.replace(",", "").replace("%", "")
                        )
                    if isinstance(stressed_value, str):
                        stressed_value = float(
                            stressed_value.replace(",", "").replace("%", "")
                        )

                    # Ratios → percentage scale
                    if metric in [
                        "Liquidity Ratio",
                        "Capital Adequacy",
                        "ROA",
            ...
            if selected_scenario:
                ...
                if scenario_key in results:
                    ...
                    with col1:
                        ...
                        st.markdown(
                            f"###### 📊 {selected_scenario} - Metric Comparison"
                        )
                        ...
                        for metric in metrics_to_show:
                            ...
                            fig = go.Figure(
                                go.Indicator(
                                    ...
                                    title={
                                        "text": f"{metric.replace('_', ' ').title()} {unit}"
                                    },
                                    delta={
                            ...
                            fig.update_layout(
                                height=200, margin=dict(l=10, r=10, t=50, b=10)
                            )
                            st.plotly_chart(fig, use_container_width=True)

                    with col2:
                        st.markdown(
                            f"###### ⚠️ {selected_scenario} - Impact Summary"
                        )

                        impact_summary = []

                        for metric, stressed_value in scenario_data.items():
                            if metric in baseline_metrics and metric not in [
                                "status",
                                "description",
                                "scenario",
                            ]:
            ...
            if len(scenarios_to_compare) >= 2:
                ...
                for scenario_display in scenarios_to_compare:
                    ...
                    if scenario_key in results:
                        ...
                        metric_names = [
                            m.replace("_", " ").title() for m in metrics_for_radar
                        ] + [metrics_for_radar[0].replace("_", " ").title()]

                        fig.add_trace(
                            go.Scatterpolar(
                                r=values, theta=metric_names, fill="toself", name=scenario_display
                            )
                        )
    ...
    def render_vulnerability_assessment(self):
        ...
        pivot_df = vuln_df.pivot(
            index="Risk Category", columns="Vulnerability", values="Vulnerability Score"
        )
        ...
        for idx, vuln in top_vulnerabilities.iterrows():
            with st.expander(
                f"#{idx+1}: {vuln['Vulnerability']} - Score: {vuln['Vulnerability Score']}/100",
                expanded=False,
            ):
                col1, col2, col3 = st.columns(3)
                ...
                with col2:
                    ...
                    st.markdown(
                        f"**Severity:** <span style='color:{severity_color};'>{vuln['Severity']}</span>",
                        unsafe_allow_html=True,
                    )
                with col3:
                ...
                st.progress(
                    vuln["Vulnerability Score"] / 100, text=f"Vulnerability Level"
                )
                ...
                recommended_action = actions.get(
                    vuln["Vulnerability"], "Review and develop mitigation strategy"
                )
        ...
        failed_scenarios = [
            ...
            if data.get("status") == "Fail" and name not in ("baseline", "_meta")
        ]

        if failed_scenarios:
            st.error(
                f"**Critical Alert:** {len(failed_scenarios)} scenarios resulted in regulatory breaches!"
            )
            ...
            for scenario in failed_scenarios:
                ...
                with st.expander(
                    f"❌ {scenario.replace('_', ' ').title()} - FAILED", expanded=True
                ):
                    col1, col2, col3 = st.columns(3)

                    base_liq = results["baseline"]["liquidity_ratio"]
                    base_cap = results["baseline"]["capital_adequacy"]

                    st_liq = scenario_data.get("liquidity_ratio", 0)
                    st_cap = scenario_data.get("capital_adequacy", 0)

                    with col1:
                        st.metric(
                            "Liquidity Ratio",
                            f"{st_liq*100:.1f}%",
                            delta=f"{(st_liq - base_liq)*100:.1f}%",
                        )
                    ...
                    with col2:
                        st.metric(
                            "Capital Adequacy",
                            f"{st_cap*100:.1f}%",
                            delta=f"{(st_cap - base_cap)*100:.1f}%",
                        )
    ...
    def render_contingency_planning(self):
        ...
        with col_gen1:
            if st.button("📋 Generate Executive Summary", use_container_width=True):
                ...
                st.info(
                    "Includes: Key findings, Top vulnerabilities, Recommended actions"
                )
        ...
        with col_gen2:
            if st.button("🏛️ Generate Board Report", use_container_width=True):
                ...
                st.info(
                    "Includes: Detailed analysis, Scenario impacts, Strategic recommendations"
                )
        ...
        with col_gen3:
            if st.button("📊 Generate ICAAP Report", use_container_width=True):
                ...
                st.info(
                    "Includes: Regulatory compliance, Capital adequacy assessment, Stress testing results"
                )
