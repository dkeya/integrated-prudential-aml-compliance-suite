# core/analytics/policy_engine.py
...
class PolicyEngineAnalyzer:
    ...
    def __init__(self, db_connection: Optional[Any] = None):
        ...
        self.policy_rules: List[PolicyRule] = self._load_policy_rules()
        self.rule_executions: List[RuleExecution] = []
        self.active_alerts: List[PolicyAlert] = []

    def _load_policy_rules(self) -> List[PolicyRule]:
        """Load policy rules from configuration"""
        try:
            # Load from YAML configuration (placeholder – currently returns defaults)
                ...
                rules.append(
                    PolicyRule(
                        ...
                        created_date=datetime.strptime(
                            rule_config["created_date"], "%Y-%m-%d"
                        ),
                        last_triggered=datetime.strptime(
                            rule_config["last_triggered"], "%Y-%m-%d"
                        )
                        if rule_config["last_triggered"]
                        else None,
    ...
    def _load_rules_config(self) -> List[Dict[str, Any]]:
        """Load rules configuration from YAML (currently returns in-code defaults)"""
        default_rules = [
            {
                "rule_id": "RULE-001",
    ...
    def execute_policy_engine(self) -> Dict[str, Any]:
        ...
        """
        try:
            execution_results: List[RuleExecution] = []
            triggered_rules: List[PolicyRule] = []
            generated_alerts: List[PolicyAlert] = []
    ...
    def _execute_rule(self, rule: PolicyRule) -> RuleExecution:
        ...
        try:
            ...
            triggered = any(
                metric.get("triggered", False) for metric in conditions_met.values()
            )
    ...
    def _get_current_metrics(self) -> Dict[str, float]:
        ...
        try:
            # Simulated metrics – in production this would query actual data via DatabaseManager
            return {
                "par_30_days": 3.2,
                ...
                "total_loans": 1_800_000_000,
                "total_deposits": 2_200_000_000,
            }
        except Exception as e:
    ...
    def _generate_alert(
        ...
        try:
            return PolicyAlert(
                ...
                description=(
                    f"Policy rule {rule.rule_name} has been triggered. {rule.description}"
                ),
    ...
    def _generate_execution_summary(
        ...
        return {
            ...
            "execution_success_rate": (
                successful_executions / total_rules * 100 if total_rules > 0 else 0
            ),
            "trigger_rate": (
                triggered_rules / total_rules * 100 if total_rules > 0 else 0
            ),
            "last_execution_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        }

    def _analyze_rule_effectiveness(
        self,
        executions: List[RuleExecution],
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for rule_id, perf in rule_performance.items():
                if perf["total_executions"] > 0:
                    trigger_rate = (
                        perf["triggered_count"] / perf["total_executions"]
                    ) * 100
                    success_rate = (
                        perf["success_count"] / perf["total_executions"]
                    ) * 100
            ...
            return {
                ...
                "overall_effectiveness": (
                    np.mean(list(effectiveness_scores.values()))
                    if effectiveness_scores
                    else 0
                ),
                "most_effective_rule": (
                    max(effectiveness_scores, key=effectiveness_scores.get)
                    if effectiveness_scores
                    else None
                ),
            }
    ...
    def _calculate_risk_exposure(
        ...
        try:
            ...
            max_possible_score = len(triggered_rules) * 1.0  # all critical
            ...
            return {
                ...
                "critical_alerts": len(
                    [r for r in triggered_rules if r.severity == AlertSeverity.CRITICAL]
                ),
                "high_alerts": len(
                    [r for r in triggered_rules if r.severity == AlertSeverity.HIGH]
                ),
                "risk_level": (
                    "Critical"
                    if risk_exposure >= 80
    ...
    def _assess_compliance_status(
        ...
        try:
            # NOTE: Original logic used rule_id text; kept as-is for now
            compliance_rules = [
                e
                for e in executions
                if any(word in e.rule_id for word in ["COMPLIANCE", "REGULATORY"])
            ]
            triggered_compliance = len([e for e in compliance_rules if e.triggered])

            total_compliance = len(compliance_rules)
            compliance_rate = (
                (total_compliance - triggered_compliance)
                / total_compliance
                * 100
    ...
    def _generate_policy_recommendations(
        ...
        # Analyze frequently triggered rules (placeholder logic)
        frequent_triggers: List[str] = []
        for execution in executions:
            if execution.triggered:
        ...
        for execution in executions:
            rule = next(
                (r for r in self.policy_rules if r.rule_id == execution.rule_id), None
            )
            if rule:
