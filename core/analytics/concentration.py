# core/analytics/concentration.py
...
class ConcentrationAnalyzer:
    ...
    def __init__(self):
        # Placeholders for unified core services (currently not used directly in logic)
                ...
                loans.append(
                    {
                        ...
                        'employer_name': np.random.choice(
                            employers,
                            p=self._generate_employer_probabilities(len(employers))
                        ),
                        ...
                        'region': np.random.choice(
                            regions,
                            p=[0.40, 0.15, 0.10, 0.10, 0.15, 0.05, 0.05]
                        ),
                        ...
                        'origination_date': datetime(2022, 1, 1)
                        + timedelta(days=np.random.randint(0, 800))
                    }
                )
    ...
    def _extract_employer_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            employer_data: List[Dict[str, Any]] = []
    ...
    def _analyze_employer_concentration(
        self,
        ...
        try:
            ...
            employer_exposures = (
                loan_data.groupby('employer_name')
                .agg(
                    {
            ...
            employer_exposures['exposure_share'] = (
                employer_exposures['outstanding_amount'] / total_portfolio
            )
            ...
            single_largest_share = (
                employer_exposures.iloc[0]['exposure_share'] if len(employer_exposures) > 0 else 0
            )
            ...
            return {
                ...
                'significant_exposures': len(
                    employer_exposures[employer_exposures['exposure_share'] > 0.05]
                )
            }
        except Exception as e:
    ...
    def _analyze_product_concentration(
        self,
        loan_data: pd.DataFrame,
    ...
    def _analyze_geographic_concentration(
        self,
        loan_data: pd.DataFrame,
        ...
        try:
            ...
            regional_exposures['exposure_share'] = (
                regional_exposures['outstanding_amount'] / total_portfolio
            )

            # Create geographic concentration analysis
            geographic_concentration: Dict[str, Dict[str, Any]] = {}
            for _, row in regional_exposures.iterrows():
                ...
                if not region_filter.empty:
                    ...
                    geographic_concentration[row['region']] = {
                        ...
                        'average_balance': (
                            row['outstanding_amount'] / row['loan_id'] if row['loan_id'] > 0 else 0
                        ),
    ...
    def _identify_regulatory_breaches(self, loan_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
                ...
                breaches.append(
                    {
                        ...
                        'description': (
                            f'Single employer exposure ({single_largest*100:.1f}%) '
                            f'exceeds {self.regulatory_limits["single_employer_max"]*100:.1f}% limit'
                        )
                    }
                )
            ...
            top_5_share = (
                employer_shares.nlargest(5).sum()
                if len(employer_shares) >= 5
                else employer_shares.sum()
            )
                ...
                breaches.append(
                    {
                        ...
                        'description': (
                            f'Top 5 employer exposure ({top_5_share*100:.1f}%) '
                            f'exceeds {self.regulatory_limits["top_5_employers_max"]*100:.1f}% limit'
                        )
                    }
                )
    ...
    def _assess_overall_risk(self, loan_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            product_risk = min(
                product_analysis.get('concentration_risk', {}).get('herfindahl_index', 0) / 0.25,
                1.0
            )
            ...
            largest_geo_share = (
                max([x.get('exposure_share', 0) for x in geographic_values]) if geographic_values else 0
            )
    ...
    def _calculate_product_risk_score(
        self,
        product_shares: pd.Series,
        product_quality: Dict[str, Dict[str, float]]
    ) -> float:
        ...
        try:
            ...
            if len(product_shares) > 0 and product_shares.max() > 0.4:
                ...
                recommendations.append(
                    {
                        'action': (
                            f'Reduce concentration in {dominant_product} '
                            f'(current: {product_shares.max()*100:.1f}%)'
                        ),
                        'priority': 'High',
                        'impact': 'High'
                    }
            ...
            for product, quality in product_quality.items():
                if quality.get('par_30', 0) > 0.08 and product_shares.get(product, 0) > 0.1:
                    recommendations.append(
                        {
                            'action': (
                                f'Review underwriting for {product} '
                                f'(PAR 30: {quality["par_30"]*100:.1f}%)'
                            ),
                            'priority': 'Medium',
                            'impact': 'Medium'
                        }
    ...
    def _identify_key_concerns(
        self,
        employer_analysis: Dict[str, Any],
        ...
        try:
            ...
            if single_largest > 0.15:
                concerns.append(
                    f"Single employer exposure ({single_largest*100:.1f}%) approaching regulatory limit"
                )
            ...
            if geographic_analysis:
                largest_region = max(
                    geographic_analysis.values(),
                    key=lambda x: x.get('exposure_share', 0)
                )
                largest_share = largest_region.get('exposure_share', 0)
                if largest_share > 0.4:
                    region_name = list(geographic_analysis.keys())[0]
                    concerns.append(
                        f"High geographic concentration in {region_name} ({largest_share*100:.1f}%)"
                    )
        ...
        try:
            ...
            for i in range(4):
                ...
                trend_analysis[period] = {
                    ...
                    'single_largest_share': max(
                        0,
                        base_analysis.get('single_largest_share', 0) + variation / 2
                    ),
                    'herfindahl_index': max(
                        0,
                        base_analysis.get('herfindahl_index', 0) + variation / 3
                    )
                }
            ...
            return {
                ...
                'regulatory_compliance': {
                    'single_employer_breach': (
                        base_analysis.get('single_largest_share', 0)
                        > self.regulatory_limits['single_employer_max']
                    ),
                    'top_5_breach': (
                        base_analysis.get('top_5_share', 0)
                        > self.regulatory_limits['top_5_employers_max']
                    )
                }
            }
        except Exception as e:
    ...
    def analyze_regulatory_compliance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            compliance_trends: Dict[str, Dict[str, float]] = {}
            for i in range(4):
                period = f'Q{4 - i} 2023'
    ...
    def calculate_early_warning_indicators(self) -> Dict[str, Any]:
        ...
        try:
            ...
            indicators: Dict[str, Any] = {
                ...
                'limit_utilization': (
                    employer_analysis.get('single_largest_share', 0)
                    / self.regulatory_limits['single_employer_max']
                ),
        ...
        try:
            ...
            return {
                ...
                'product_metrics': {
                    ...
                    'dominant_product_share': product.get('concentration_risk', {}).get(
