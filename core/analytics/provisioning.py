# core/analytics/provisioning.py
import pandas as pd
import numpy as np
...
class ProvisioningAnalyzer:
    ...
    def calculate_ifrs9_ecl(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if loan_data.empty:
                ...
                # Create sample data for testing / dev
            ...
            'staging_details': [
                {
                    ...
                    'risk_composition': {
                        ...
                        'average_employer_stability': 0.85,
                    },
                },
                {
                    'stage': 'Stage 2',
                    ...
                    'average_dpd': 45.8,
                    'risk_composition': {
                        'high_risk_share': 0.45,
                        'collateral_coverage': 0.95,
                        'average_employer_stability': 0.65,
                    },
                },
                {
                    'stage': 'Stage 3',
                    ...
                    'average_dpd': 120.5,
                    'risk_composition': {
                        'high_risk_share': 0.85,
                        'collateral_coverage': 0.75,
                        'average_employer_stability': 0.45,
                    },
                },
            ],
    ...
    def _create_sample_full_analysis(self) -> Dict[str, Any]:
        """Create complete sample analysis for testing (used on hard failure)"""
        staging_analysis = self._create_sample_staging_analysis()
        ecl_calculations = self._create_sample_ecl_calculations()
                ...
                'journal_entries': [
                    {
                        ...
                        'amount': 4080000,
                        'description': 'ECL Provision - Stage 1',
                    },
                    {
                        'account_debit': 'Provision for Loan Losses',
                        'account_credit': 'Loan Loss Reserve',
                        'amount': 21037500,
                        'description': 'ECL Provision - Stage 2',
                    },
                    {
                        'account_debit': 'Provision for Loan Losses',
                        'account_credit': 'Loan Loss Reserve',
                        'amount': 35000000,
                        'description': 'ECL Provision - Stage 3',
                    },
                ],
            },
            'portfolio_ecl': {
                ...
                'average_lgd': 0.55,
                'ecl_trend': {
                    '3_months_ago': 58000000,
                    '2_months_ago': 59000000,
                    '1_month_ago': 59500000,
                    'current': 60117500,
                },
                'stress_test_results': {
                    'base_scenario_ecl': 60117500,
                    'adverse_scenario_ecl': 72141000,
                    'ecl_increase_percentage': 20.0,
                },
            },
            'writeoff_analysis': {
    ...
    def _extract_loan_portfolio_data(self) -> pd.DataFrame:
        """Extract comprehensive loan portfolio data (simulated for now)"""
        try:
            np.random.seed(42)
            n_loans = 5000
    ...
    def _extract_macroeconomic_data(self) -> Dict[str, float]:
        """Extract current macroeconomic indicators (simulated)"""
        try:
    ...
    def _extract_payment_history(self) -> pd.DataFrame:
        """Extract loan payment history for behavioral analysis (simulated)"""
        try:
            np.random.seed(42)
            n_payments = 50000
        ...
        try:
            ...
            for stage_detail in staging_analysis.get('staging_details', []):
                ...
                if stage == LoanStage.STAGE_1.value:
                    ...
                    # Stage 2 and Stage 3 both use lifetime PD multipliers here
    ...
    def _calculate_provisioning_requirements(self, ecl_calculations: List[ECLCalculation]) -> Dict[str, Any]:
        ...
        try:
            ...
            provision_by_stage: Dict[str, float] = {}
    ...
    def _analyze_writeoff_potential(self, staging_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            writeoff_potential = sum(
                loan['outstanding_amount'] for loan in high_risk_loans
            ) * 0.7  # 70% write-off rate
    ...
    def _calculate_ecl_trend(self) -> Dict[str, float]:
        """Calculate ECL trend over time (simulated)"""
        return {
    ...
    def _calculate_writeoff_trend(self) -> Dict[str, float]:
        """Calculate write-off trend over time (simulated)"""
        return {
    ...
    def _get_empty_staging_analysis(self) -> Dict[str, Any]:
        """Empty staging structure used on failure"""
        return {
            'stage_summary': {},
            'staging_details': [],
