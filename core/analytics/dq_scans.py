...
from core.analytics.dq import DataQualityAnalyzer
...
class DataQualityScanner:
    ...
    def __init__(self, config_manager: Optional[ConfigManager] = None):
        ...
        self.alert_settings: Dict[str, Any] = {}
    ...
    def get_scan_statistics(self) -> Dict[str, Any]:
        ...
        active_scans = len(
            [s for s in self.scheduled_scans.values() if s['status'] == 'Active']
        )
        ...
        return pd.DataFrame(
            {
                ...
                # Optional coverage for the trends chart
                'coverage': np.clip(
                    np.linspace(80, 98, len(dates))
    ...
    def run_comprehensive_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            scan_result = {
                ...
                'issues_found': len(issues_report.get('critical_issues', []))
                + len(issues_report.get('quality_warnings', [])),
    ...
    def run_scheduled_scan(self, scan_name: str) -> Dict[str, Any]:
        ...
        scan_config = next(
            (s for s in self.scheduled_scans.values() if s['name'] == scan_name), None
        )
    ...
    def test_custom_rule(self, rule_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if test_passed:
                return {
                    'success': True,
                    'message': 'Rule test passed - no issues found',
                }
            else:
                return {
                    'success': False,
                    'message': 'Rule test failed - issues detected',
                }
    ...
    def apply_scan_template(self, template_name: str) -> Dict[str, Any]:
        ...
        if self.save_scheduled_scan(scan_config):
            ...
            return {
                'success': False,
                'error': 'Failed to create scheduled scan from template',
            }
    ...
    def get_scan_history(self, days: int = 30) -> pd.DataFrame:
        ...
        Returns a DataFrame with the columns expected by the
        07A_Data_Quality_Scans page, including severity breakdown:
        - critical_issues
        - high_issues
        - medium_issues
        - low_issues
        """
        # Limit how far back we go to keep things reasonable
        ...
        # Generate severity-level issue counts
        np.random.seed(42)
        critical = np.random.poisson(1, n)
        high = np.random.poisson(3, n)
        medium = np.random.poisson(5, n)
        low = np.random.poisson(8, n)

        issues_found = critical + high + medium + low

        scan_types = np.random.choice(
            ['Full Scan', 'Quick Scan', 'Scheduled Scan', 'AI Scan'], n
        )
        ...
        success_rate = np.clip(np.random.normal(93, 3, n), 80, 100)

        duration_seconds = np.random.uniform(45, 300, n)
        tables_scanned = np.random.randint(3, 8, n)

        return pd.DataFrame(
            {
                'scan_id': [f'SCAN_{i:04d}' for i in range(1, n + 1)],
                'timestamp': dates,
