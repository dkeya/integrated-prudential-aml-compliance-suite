...
class GovernanceAnalyzer:
    ...
    def __init__(self):
        # Unified core services (placeholders for future DB/config usage)
    ...
    def get_compliance_trends(self, months: int = 8) -> pd.DataFrame:
        ...
        try:
            ...
            trend_data = {
                ...
                'Overall_Compliance': (base_compliance + compliance_noise)
                .clip(75, 95)
                .round(1)
                .tolist(),
                'Capital_Adequacy': (
            ...
            for likelihood in likelihood_levels:
                count = len(
                    [
                        r
                        for r in risks_data
                        if r.get('impact') == impact
                        and r.get('likelihood') == likelihood
                    ]
                )
                heat_data.append(
                    {
    ...
    def track_audit_findings(self, findings_data: List[Dict]) -> Dict[str, Any]:
        ...
        high_severity = len(
            [f for f in findings_data if f.get('severity') == 'High']
        )
        medium_severity = len(
            [f for f in findings_data if f.get('severity') == 'Medium']
        )
        low_severity = len(
            [f for f in findings_data if f.get('severity') == 'Low']
        )

        # Track remediation status
        completed = len(
            [f for f in findings_data if f.get('status') == 'Completed']
        )
        in_progress = len(
            [f for f in findings_data if f.get('status') == 'In Progress']
        )
        not_started = len(
            [f for f in findings_data if f.get('status') == 'Not Started']
        )
        overdue = len(
            [f for f in findings_data if f.get('status') == 'Overdue']
        )
        ...
        if total_findings > 0:
            incomplete_penalty = (
                (total_findings - completed) / total_findings
            ) * 50
        else:
    ...
    def monitor_policy_compliance(self, policies_data: List[Dict]) -> Dict[str, Any]:
        ...
        current_policies = len(
            [p for p in policies_data if p.get('status') == 'Current']
        )
        update_due = len(
            [p for p in policies_data if p.get('status') == 'Update Due']
        )
        expired = len(
            [p for p in policies_data if p.get('status') == 'Expired']
        )
        ...
        avg_days_since_review = (
            float(np.mean(days_since_review)) if days_since_review else 0.0
        )

        # Policy health score with zero division protection
        currency_score = (
            (current_policies / total_policies) * 100 if total_policies > 0 else 100
        )
        # Penalize older reviews
        review_score = max(100 - (avg_days_since_review / 365 * 100), 0)
        ...
        while current_date <= end:
            for report in regulatory_reports:
                if report['frequency'] == 'quarterly':
                    ...
                    # Specific day of year (hard-coded June 30th for now)
                    annual_date = datetime(current_date.year, 6, 30)
                    if start <= annual_date <= end:
                        calendar_events.append(
                            {
                                'report': report['report'],
