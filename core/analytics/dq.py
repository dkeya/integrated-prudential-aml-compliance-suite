...
class DataQualityAnalyzer:
    ...
    def _assess_dataset_quality(self, dataset_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Dataset quality assessment failed for {dataset_name}: {e}")
            return {
                'dataset': dataset_name,
                'error': str(e),
                'completeness': 0,
    ...
    def _calculate_consistency(self, df: pd.DataFrame, dataset_name: str) -> float:
        ...
        try:
            ...
            if dataset_name == 'members':
                ...
                if loans_df is not None and 'member_id' in loans_df.columns and 'member_id' in df.columns:
                    consistency_checks += 1
            ...
            if missing_count > 0:
                ...
                issues.append(
                    DataQualityIssue(
                        ...
                        impact_score=missing_count / len(df) * 100 if len(df) > 0 else 0,
                ...
                if invalid_ids.any():
                    issues.append(
                        DataQualityIssue(
                            ...
                            impact_score=invalid_ids.sum() / len(df) * 100 if len(df) > 0 else 0,
                            detected_at=datetime.now()
                        )
                    )
            ...
            for dataset in datasets:
                ...
                if df is not None and not df.empty:
                    ...
                    for dtype, count in dataset_profile.get('data_types', {}).items():
                        profiling_results['data_types'][dtype] = (
                            profiling_results['data_types'].get(dtype, 0) + count
                        )
    ...
    def _check_validation_rule(self, rule: Dict[str, Any]) -> str:
        ...
        try:
            df = self._load_dataframe(rule['table_name'])
            if df is None:
                return 'Unknown'
    ...
    def generate_data_quality_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'executive_summary': {
                ...
                'data_health': (
                    'Excellent'
                    if assessment.get('overall_score', 0) > 0.9 else
                    'Good'
                    if assessment.get('overall_score', 0) > 0.8 else
                    'Fair'
                    if assessment.get('overall_score', 0) > 0.7 else
                    'Poor'
                ),
    ...
    def get_data_quality_trends(self, months: int = 7) -> pd.DataFrame:
        ...
        try:
            ...
            trend_data = {
                ...
                'Completeness_Score': (
                    scores * 0.95 + np.random.normal(0, 3, n_months)
                ).clip(75, 98).round(1).tolist(),
                'Accuracy_Score': (
                    scores * 0.92 + np.random.normal(0, 3, n_months)
                ).clip(70, 96).round(1).tolist()
            }
            ...
            logger.error(f"Error generating data quality trends: {e}")
            # Fallback to simple mock data with guaranteed consistent lengths
            months_list = ['Aug 2023', 'Sep 2023', 'Oct 2023',
