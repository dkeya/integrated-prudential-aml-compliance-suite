...
from sacco_core.aml.investigations import (
    ...
    update_status, update_summary,
    add_comment, list_comments, timeline,
    save_attachment, list_attachments,
...
st.set_page_config(
    page_title="MLRO Portal — Case Investigations",
    page_icon="🏛️",
    layout="wide",
...
# Global filters row (for context; cases list has its own filters below)
...
with tabs[0]:
    ...
    status_sel = c1.multiselect("Status", ["Open", "Under Review", "Escalated", "Closed"], default=["Open", "Under Review"])
    severity_sel = c2.multiselect("Severity", ["Low", "Medium", "High", "Critical"], default=[])
    d_from = c3.date_input("From", value=None)
    d_to = c4.date_input("To", value=None)
    q = c5.text_input("Search (title / entity / summary)", value="")
    ...
    if df.empty:
        ...
        sel = st.selectbox("Open case id", [""] + df["case_id"].astype(str).tolist())
        if sel:
...
with tabs[1]:
    ...
    with st.form("new_case"):
        ...
        severity = col2.selectbox("Severity", ["Low", "Medium", "High", "Critical"], index=1)
        category = col3.selectbox("Category", ["Sanctions", "Alerts", "KYC", "Transaction", "Other"], index=4)
        ...
        if submit:
            if not title.strip():
                ...
                cid = create_case(
                    title=title.strip(),
                    created_by=st.session_state.get("user_role", "MLRO"),
...
with tabs[2]:
    ...
    if hits.empty:
        st.info("No sanctions dispositions to promote. Use the Sanctions page first.")
    else:
        st.caption("Tip: Filter in the Sanctions page, record disposition (Open/Escalated), then create a case here.")
        st.dataframe(hits, use_container_width=True, height=360)
        idx = st.selectbox("Pick screening row index", [""] + hits.index.astype(str).tolist())
        if idx:
            row = hits.loc[int(idx)].to_dict()
            if st.button("Create Case from Selected Hit", type="primary", use_container_width=True):
                cid = create_case_from_sanction(row, created_by=st.session_state.get("user_role", "MLRO"))
                st.session_state["selected_case_id"] = cid
    ...
    if not cid:
        ...
        if case.empty:
            ...
            new_status = w1.selectbox(
                ...
                index=["Open", "Under Review", "Escalated", "Closed"].index(str(row["status"]))
            )
            if w2.button("Update", use_container_width=True):
                update_status(int(cid), st.session_state.get("user_role", "MLRO"), new_status)
                st.success("Status updated.")
                st.experimental_rerun()

            # Summary editable
            st.markdown("#### 📝 Summary")
            new_sum = st.text_area("Summary / Narrative", value=str(row.get("summary") or ""), height=140)
            ...
            with st.form("add_comment"):
                ...
                if st.form_submit_button("Post"):
                    if txt.strip():
                        add_comment(int(cid), st.session_state.get("user_role", "MLRO"), txt.strip())
            ...
            if comm.empty:
                ...
                st.dataframe(comm, use_container_width=True, height=240)

            # Attachments
            st.markdown("#### 📎 Attachments")
            up = st.file_uploader("Upload file (PDF, DOCX, XLSX, images…)", type=None)
            if up is not None:
                data = up.read()
                rel = save_attachment(int(cid), up.name, data)
            ...
            if att.empty:
                ...
                st.dataframe(att, use_container_width=True, height=240)
