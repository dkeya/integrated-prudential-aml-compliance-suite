...
class PolicyIntelligencePage:
    ...
    def render_compliance_radar_marquee(self):
        ...
        try:
            ...
            risk_exposure = self.analysis['risk_exposure'].get('risk_exposure_percentage', 0)
    ...
    def render_policy_intelligence_dashboard(self):
        ...
        # Strategic 5-tab structure for policy intelligence
    ...
    def render_policy_intelligence_framework(self):
        ...
        with st.expander("🧠 POLICY INTELLIGENCE FRAMEWORK", expanded=False):
            ...
            with col1:
                ...
                st.info("Real-time monitoring of policy health, rule triggers, and compliance signals")
                st.markdown("**⚙️ Rule Engine Ops**")
                st.info("Automated rule execution, effectiveness tracking, and performance optimization")
            
            with col2:
                st.markdown("**📊 Compliance Intelligence**")
                st.info("Multi-regulatory framework compliance tracking and gap analysis")
                st.markdown("**🔄 Control Operations**")
                st.info("Control effectiveness monitoring and continuous improvement cycles")
            
            with col3:
                st.markdown("**🎯 Regulatory Command**")
                st.info("Centralized regulatory requirement management and audit readiness")
                st.markdown("**🤖 Predictive Compliance**")
                st.info("AI-driven compliance risk prediction and proactive control enhancement")
    ...
    def render_policy_radar(self):
        ...
        with col1:
            ...
            # Policy clusters
            for i, cluster in enumerate(policy_data['clusters']):
        ...
        with col2:
            ...
            with col_indicators[0]:
                st.metric("Policy Health", f"{st.session_state.live_policy_pulse['policy_health']}%")
                st.metric("Rule Velocity", st.session_state.live_policy_pulse['rule_velocity'])
            with col_indicators[1]:
                st.metric("Control Coverage", f"{st.session_state.live_policy_pulse['control_coverage']}%")
                st.metric("Compliance Momentum", f"{st.session_state.compliance_signals['compliance_velocity']}")
            ...
            if st.button("📡 Deep Policy Analysis", use_container_width=True):
                st.warning("Deep policy analysis running! Scanning all policy frameworks.")
            
            if st.button("🚨 Emergency Override", use_container_width=True, type="secondary"):
            ...
            for policy in st.session_state.live_policy_pulse['hot_policies']:
                severity_color = "#dc2626" if policy['severity'] == 'High' else "#f59e0b" if policy['severity'] == 'Medium' else "#22c55e"
                st.markdown(f"""
            ...
            policy_protocol = st.selectbox(
                ...
                ["None", "Code Green: Normal Operations", "Code Yellow: Enhanced Monitoring", 
                 "Code Orange: Policy Review", "Code Red: Emergency Controls"]
            )
            
            if policy_protocol != "None" and st.button("⚡ Execute Protocol", use_container_width=True):
    ...
    def _generate_policy_landscape(self):
        ...
        # Create clusters for different policy areas
        clusters = []
        
        # Credit Risk - High compliance
        clusters.append({
            ...
            'color': '#059669',
            'policies': ['Single Borrower Limit', 'Portfolio Concentration', 'PAR Monitoring', 'Provisioning Policy']
        })
        
        # Liquidity Management - Medium compliance
        clusters.append({
            ...
            'color': '#0ea5e9',
            'policies': ['Liquidity Coverage', 'Cash Reserve', 'ALM Policy', 'Stress Testing']
        })
        
        # Operational Risk - Lower compliance
        clusters.append({
            ...
            'color': '#f59e0b',
            'policies': ['Fraud Prevention', 'BCP Policy', 'Data Protection', 'Internal Controls']
        })
        
        # Risk vectors (areas needing attention)
        risk_vectors = [
            {
                'x': [40, 55, 70],
                'y': [35, 50, 65],
    ...
    def _generate_policy_heatmap(self):
        ...
        policy_areas = ['Credit Risk', 'Liquidity', 'Operational', 'Market Risk', 'Compliance']
        regulatory_frameworks = ['SASRA', 'CBK', 'IFRS 9', 'Basel III', 'Data Protection']
        
        # Generate effectiveness matrix
        effectiveness_matrix = np.array([
            [92, 88, 85, 82, 90],   # Credit Risk
            [88, 85, 80, 78, 82],   # Liquidity
            [78, 75, 72, 70, 68],   # Operational
            [85, 82, 80, 78, 75],   # Market Risk
            [95, 92, 90, 88, 85]    # Compliance
        ])
    ...
    def render_rule_engine_ops(self):
        ...
        with col1:
            # Rule engine command dashboard
            st.markdown("##### ⚙️ RULE ENGINE COMMAND DASHBOARD")
            
            # Current engine status
            current_status = {
                'total_rules': len(self.engine.policy_rules) if hasattr(self.engine, 'policy_rules') else 0,
                'execution_success': self.analysis['execution_summary'].get('execution_success_rate', 0),
                'trigger_rate': self.analysis['execution_summary'].get('trigger_rate', 0),
            ...
            optimization_options = [
                ...
                {"option": "Compliance Rule Enhancement", "impact": "+12.2%", "effort": "High"},
                {"option": "Performance Optimization", "impact": "+5.8%", "effort": "Low"},
                {"option": "Maintain Current", "impact": "0.0%", "effort": "None"}
            ]
            
            for option in optimization_options:
                impact_color = "#059669" if float(option['impact'].replace('+', '').replace('%', '')) > 8 else "#f59e0b"
                st.markdown(f"""
        ...
        with col2:
            ...
            category_data = [
                {"category": "Credit Risk Rules", "effectiveness": 92.4, "alerts": 24, "trend": "↑"},
                {"category": "Compliance Rules", "effectiveness": 88.7, "alerts": 18, "trend": "→"},
                {"category": "Operational Rules", "effectiveness": 78.9, "alerts": 12, "trend": "↓"},
                {"category": "Security Rules", "effectiveness": 95.2, "alerts": 8, "trend": "↑"}
            ]
            ...
            for category in category_data:
                ...
                with col_cat[1]:
                    trend_color = "#059669" if category['trend'] == "↑" else "#dc2626" if category['trend'] == "↓" else "#f59e0b"
                    st.metric("", f"{category['effectiveness']:.1f}%", category['trend'], label_visibility="collapsed")
                with col_cat[2]:
    ...
    def _generate_rule_performance(self):
        ...
        # Trigger rates with volatility
        trigger_rates = [12.5 + np.sin(w * 0.5) * 3 + np.random.normal(0, 1) for w in weeks]
        
        # False positive rates (improving trend)
        false_positive_rates = [15.0 - w * 0.5 + np.random.normal(0, 0.5) for w in weeks]
    ...
    def render_compliance_intelligence(self):
        ...
        with col1:
            ...
            framework_compliance = [
                {"framework": "SASRA Prudential", "compliance": 96.5, "trend": "↑", "requirements": 42},
                {"framework": "CBK Regulations", "compliance": 94.2, "trend": "→", "requirements": 28},
                {"framework": "IFRS 9", "compliance": 98.8, "trend": "↑", "requirements": 18},
                {"framework": "Data Protection", "compliance": 92.4, "trend": "↑", "requirements": 24},
                {"framework": "Basel III", "compliance": 89.7, "trend": "↓", "requirements": 35},
                {"framework": "Anti-Money Laundering", "compliance": 97.2, "trend": "↑", "requirements": 22}
            ]
            
            for framework in framework_compliance:
                with st.container():
                    trend_color = "#059669" if framework['trend'] == "↑" else "#dc2626" if framework['trend'] == "↓" else "#f59e0b"
        ...
        with col2:
            ...
            compliance_gaps = [
                {"gap": "Basel III - Leverage Ratio", "severity": "High", "days_open": 45, "owner": "Risk Dept"},
                {"gap": "SASRA - Internal Audit Frequency", "severity": "Medium", "days_open": 28, "owner": "Audit"},
                {"gap": "Data Protection - Data Retention", "severity": "Medium", "days_open": 32, "owner": "IT"},
                {"gap": "CBK - Stress Testing Scenarios", "severity": "Low", "days_open": 15, "owner": "Treasury"}
            ]
            
            for gap in compliance_gaps:
                severity_color = "#dc2626" if gap['severity'] == 'High' else "#f59e0b" if gap['severity'] == 'Medium' else "#22c55e"
            ...
            selected_gap = st.selectbox(
                ...
                ["Basel III - Leverage Ratio", "SASRA - Internal Audit Frequency", "Data Protection - Data Retention"]
            )
            
            if st.button("⚡ Activate Resolution", use_container_width=True):
                st.success(f"Resolution activated for {selected_gap}! Task assigned to owner.")
    
    def render_control_ops(self):
        """Render control operations center"""
        st.markdown("##### 🔄 CONTROL OPERATIONS CENTER")
        
        tab1, tab2, tab3 = st.tabs(["🛡️ Control Effectiveness", "📊 Performance Ops", "🧪 Testing Lab"])
        ...
        with tab1:
            ...
            control_metrics = [
                {"control": "Automated Monitoring", "effectiveness": 94, "target": 95, "trend": "↑"},
                {"control": "Manual Reviews", "effectiveness": 82, "target": 90, "trend": "→"},
                {"control": "Segregation of Duties", "effectiveness": 98, "target": 100, "trend": "↑"},
                {"control": "Access Controls", "effectiveness": 96, "target": 98, "trend": "↑"}
            ]
            ...
            for control in control_metrics:
                ...
                with col_ctrl[1]:
                    trend_color = "#059669" if control['trend'] == "↑" else "#dc2626" if control['trend'] == "↓" else "#f59e0b"
                    st.metric("", f"{control['effectiveness']}%", control['trend'], label_visibility="collapsed")
                with col_ctrl[2]:
                    progress = (control['effectiveness'] / control['target']) * 100
                    progress_color = "#059669" if progress >= 95 else "#f59e0b" if progress >= 85 else "#dc2626"
        ...
        with tab2:
            ...
            performance_ops = [
                {"operation": "Control Testing", "status": "Active", "success_rate": 92, "coverage": 85},
                {"operation": "Exception Monitoring", "status": "Active", "success_rate": 88, "coverage": 78},
                {"operation": "Control Automation", "status": "Testing", "success_rate": 75, "coverage": 45},
                {"operation": "Remediation Tracking", "status": "Active", "success_rate": 95, "coverage": 92}
            ]
            ...
            for op in performance_ops:
                ...
                with col_ops[0]:
                    ...
                    status_color = "#059669" if op['status'] == 'Active' else "#f59e0b" if op['status'] == 'Testing' else "#dc2626"
                    st.markdown(f"<span style='color: {status_color};'>Status: {op['status']}</span>", unsafe_allow_html=True)
                with col_ops[1]:
                    st.metric("Success", f"{op['success_rate']}%", f"Coverage: {op['coverage']}%")
        ...
        with tab3:
            ...
            test_scenarios = {
                "Control Failure": {"detection_time": "2.8h", "recovery_time": "4.2h", "risk": "Medium"},
                "Policy Breach": {"detection_time": "1.2h", "recovery_time": "3.8h", "risk": "High"},
                "Regulatory Change": {"detection_time": "24h", "recovery_time": "72h", "risk": "Low"}
            }
    ...
    def render_regulatory_command(self):
        ...
        with col1:
            ...
            regulatory_items = [
                {"item": "SASRA Q1 Returns", "due_date": "2024-03-31", "status": "On Track", "owner": "Compliance"},
                {"item": "CBK Annual Report", "due_date": "2024-04-30", "status": "On Track", "owner": "Finance"},
                {"item": "Annual External Audit", "due_date": "2024-06-30", "status": "Preparing", "owner": "Audit"},
                {"item": "Data Protection Audit", "due_date": "2024-05-15", "status": "Behind", "owner": "IT"}
            ]
            
            for i, item in enumerate(regulatory_items, 1):
                with st.container():
                    status_color = "#059669" if item['status'] == 'On Track' else "#f59e0b" if item['status'] == 'Preparing' else "#dc2626"
        ...
        with col2:
            ...
            regulatory_updates = [
                {"update": "New CBK Circular on Cybersecurity", "impact": "High", "deadline": "2024-09-30"},
                {"update": "SASRA Guideline Amendments", "impact": "Medium", "deadline": "2024-08-15"},
                {"update": "Data Protection Regulation Updates", "impact": "High", "deadline": "2024-07-31"},
                {"update": "IFRS 9 Implementation Guidance", "impact": "Low", "deadline": "2024-06-30"}
            ]
            ...
            for update in regulatory_updates:
                ...
                with col_update[1]:
                    impact_color = "#dc2626" if update['impact'] == 'High' else "#f59e0b" if update['impact'] == 'Medium' else "#059669"
                    st.markdown(f"<span style='color: {impact_color}; font-weight: bold;'>{update['impact']}</span>", unsafe_allow_html=True)
                with col_update[2]:
    ...
    def render_enhanced_policy_dashboard(self):
        ...
        with col1:
            summary = self.analysis['execution_summary']
            success_rate = summary.get('execution_success_rate', 0)
            success_color = "#059669" if success_rate >= 95 else "#f59e0b" if success_rate >= 85 else "#dc2626"
        ...
        with col2:
            risk_exposure = self.analysis['risk_exposure']
            risk_percentage = risk_exposure.get('risk_exposure_percentage', 0)
            risk_color = "#059669" if risk_percentage < 20 else "#f59e0b" if risk_percentage < 40 else "#dc2626"
        ...
        with col3:
            compliance = self.analysis['compliance_status']
            compliance_rate = compliance.get('compliance_rate', 0)
            compliance_color = "#059669" if compliance_rate >= 95 else "#f59e0b" if compliance_rate >= 85 else "#dc2626"
        ...
        with col4:
            effectiveness = self.analysis['rule_effectiveness']
            rule_effectiveness = effectiveness.get('overall_effectiveness', 0)
            effectiveness_color = "#059669" if rule_effectiveness >= 90 else "#f59e0b" if rule_effectiveness >= 80 else "#dc2626"
    ...
    def render_detailed_sections(self):
        ...
        with tab1:
            self.display_rule_dashboard(self.engine.policy_rules if hasattr(self.engine, 'policy_rules') else [], self.analysis)
        
        with tab2:
            self.display_active_alerts(self.analysis['active_alerts'])
        ...
        with tab5:
            self.display_recommendations(self.analysis['recommendations'])
    ...
    def render_manual_execution(self):
        ...
        with col1:
            if hasattr(self.engine, 'policy_rules') and self.engine.policy_rules:
                selected_rule = st.selectbox(
                    ...
                    options=[f"{getattr(rule, 'rule_id', 'Unknown')}: {getattr(rule, 'rule_name', 'Unknown')}" 
        ...
        with col2:
            ...
            if selected_rule and st.button("🚀 Execute Selected Rule", type="primary"):
                ...
                rule = next((r for r in self.engine.policy_rules if getattr(r, 'rule_id', '') == rule_id), None)
