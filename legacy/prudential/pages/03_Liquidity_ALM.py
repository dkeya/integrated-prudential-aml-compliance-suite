...
st.set_page_config(
    page_title="🏛️ Liquidity Intelligence & ALM Analytics",
    page_icon="💧",
    layout="wide"
)
...
class LiquidityALMPage:
    ...
    def generate_enhanced_liquidity_data(self):
        ...
        liquidity_data = pd.DataFrame({
            'date': dates,
            'cash_equivalents': base_cash * growth_trend + seasonality + np.random.uniform(-3, 3, len(dates)),
            'short_term_investments': np.random.uniform(15, 25, len(dates)) * growth_trend,
            'total_deposits': base_deposits * growth_trend + seasonality * 2 + np.random.uniform(-5, 5, len(dates)),
            'loan_disbursements': np.random.uniform(15, 25, len(dates)) * growth_trend,
        ...
        liquidity_data['loan_to_deposit'] = (
            liquidity_data['loan_disbursements'].cumsum() / liquidity_data['total_deposits'] * 100
        )
        liquidity_data['cash_buffer_days'] = (
            liquidity_data['cash_equivalents'] / liquidity_data['member_withdrawals'].rolling(30).mean() * 30
        )
        
        # Add trend indicators
        liquidity_data['liquidity_trend'] = liquidity_data['liquidity_ratio'].pct_change() * 100
        liquidity_data['deposit_growth'] = liquidity_data['total_deposits'].pct_change() * 100
    ...
    def generate_enhanced_alm_data(self):
        ...
        buckets = ['Overnight', '1-7 Days', '8-30 Days', '31-90 Days', 
                  '91-180 Days', '181-365 Days', '1-3 Years', '3+ Years']
        ...
        gap_data['Rate_Sensitive_Gap'] = gap_data['Rate_Sensitive_Assets'] - gap_data['Rate_Sensitive_Liabilities']
        gap_data['Net_Interest_Margin_Impact'] = gap_data['Rate_Sensitive_Gap'] * 0.01  # 1% rate change impact
    ...
    def render_liquidity_intelligence_dashboard(self):
        ...
        with col1:
            ...
            # Add trend line
            z = np.polyfit(range(len(liquidity_data)), liquidity_data['liquidity_ratio'], 1)
            p = np.poly1d(z)
            fig.add_trace(go.Scatter(
                x=liquidity_data['date'],
            ...
            cash_components = pd.DataFrame({
                'Component': ['Cash & Equivalents', 'Short-term Investments', 'Loan Disbursements', 
                             'Member Withdrawals', 'Operational Cash Flow'],
                'Amount (KES M)': [latest['cash_equivalents'], latest['short_term_investments'],
                                  latest['loan_disbursements'], latest['member_withdrawals'],
            ...
            fig = px.bar(
                ...
                color_discrete_map={'Asset': '#0ea5e9', 'Outflow': '#ef4444', 'Net': '#10b981'},
                text=cash_components['Amount (KES M)'].apply(lambda x: f"KES {x:.1f}M")
            )
        ...
        with col2:
            ...
            for metric in metrics:
                ...
                delta_color = "inverse" if delta < 0 and metric['name'] != 'Loan-to-Deposit' else "normal"
                ...
                with col_b:
                    if metric['current'] < metric['threshold']:
                        st.markdown(f"<div style='color: #dc2626; font-size: 0.8rem;'>🔴 Below Min</div>", 
                                  unsafe_allow_html=True)
                    elif metric['current'] > metric['target']:
                        st.markdown(f"<div style='color: #10b981; font-size: 0.8rem;'>🟢 Above Target</div>", 
                                  unsafe_allow_html=True)
                    else:
                        st.markdown(f"<div style='color: #f59e0b; font-size: 0.8rem;'>🟡 Within Range</div>", 
                                  unsafe_allow_html=True)
                with col_c:
            ...
            source_data = pd.DataFrame({
                'Source': ['Member Deposits', 'Institutional Funds', 'Interbank', 
                          'Investments', 'Operational Cash'],
                'Amount (KES M)': [180.5, 65.2, 28.3, 42.8, 12.5],
                'Stability Score': [90, 75, 60, 85, 95],
                'Cost (%)': [1.2, 2.5, 3.8, 4.2, 0.5]
            })
            
            source_data['Share %'] = source_data['Amount (KES M)'] / source_data['Amount (KES M)'].sum() * 100
    ...
    def render_alm_gap_duration_analysis(self):
        ...
        with col1:
            ...
            fig.add_trace(go.Bar(
                ...
                name='Periodic Gap',
                marker_color=['red' if x < 0 else 'green' for x in gap_data['Periodic_Gap']],
            ...
            fig.add_trace(go.Scatter(
                ...
                text=gap_data['Cumulative_Gap'].apply(lambda x: f"KES {x:.1f}M"),
        ...
        with col2:
            ...
            fig.add_trace(go.Bar(
                ...
                marker_color='#3b82f6',
                text=gap_data['Rate_Sensitive_Assets'].apply(lambda x: f"{x}%"),
                textposition='inside'
            ))
            ...
            fig.add_trace(go.Bar(
                ...
                marker_color='#ef4444',
                text=gap_data['Rate_Sensitive_Liabilities'].apply(lambda x: f"{x}%"),
                textposition='inside'
            ))
            ...
            fig.add_trace(go.Bar(
                ...
                name='NIM Impact',
                marker_color=['red' if x < 0 else 'green' for x in gap_data['Net_Interest_Margin_Impact']],
                text=gap_data['Net_Interest_Margin_Impact'].apply(lambda x: f"{x:+.2f}%"),
                textposition='outside'
            ))
            ...
            duration_data = pd.DataFrame({
                'Metric': ['Asset Duration', 'Liability Duration', 'Duration Gap', 
                          'Duration of Equity', 'Immunization Gap'],
            ...
            fig = px.bar(
                ...
                color_discrete_map={'High': '#ef4444', 'Low': '#f59e0b', 'Slightly High': '#10b981'},
                text=duration_data['Value (Years)'].apply(lambda x: f"{x:.1f} yrs")
            )
    ...
    def render_funding_strategy_optimization(self):
        ...
        for strategy in strategies:
            with st.expander(f"{strategy['strategy']} - Cost: {strategy['cost']}", expanded=True):
                ...
                with col4:
                    ...
                    st.markdown(f"**Stability:** <span style='color:{stability_color};'>{strategy['stability']}</span>", 
                              unsafe_allow_html=True)
                ...
                with col_a:
                    if st.button(f"📋 Develop Plan", key=f"plan_{strategy['strategy']}"):
                        st.info(f"Strategy development started for {strategy['strategy']}")
                with col_b:
                    if st.button(f"📊 Track Progress", key=f"track_{strategy['strategy']}"):
                        st.success(f"Progress tracking initiated for {strategy['strategy']}")
    ...
    def render_liquidity_risk_management(self):
        ...
        for action in actions:
            with st.expander(f"{action['priority']} Priority: {action['risk']}", expanded=True):
                col1, col2, col3 = st.columns(3)
                ...
                with col3:
                    ...
                    st.markdown(f"**Priority:** <span style='color:{priority_color};'>{action['priority']}</span>", 
                              unsafe_allow_html=True)
                ...
                if st.button(f"📋 Update Status", key=f"status_{action['risk']}"):
    ...
    def render_treasury_performance_frameworks(self):
        ...
        performance_data = pd.DataFrame({
            'Framework': ['SASRA Liquidity', 'ALM Gap', 'Stress Testing', 'FTP'],
    ...
    def run(self):
        ...
        # Log dashboard completion - using available method
        try:
            ...
            # If for any reason log_action is not available or fails, don't break the app
            pass
