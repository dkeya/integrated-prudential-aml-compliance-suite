# pages/03A_Dividend_Capacity.py - ENHANCED WITH ENTERPRISE FEATURES
...
st.set_page_config(
    page_title="🏛️ Dividend Capacity Intelligence",
    page_icon="💰",
    layout="wide"
)
...
class DividendCapacityPage:
    ...
    def _check_access(self):
        ...
        # FIXED: Use available method instead of log_event
        try:
    ...
    def render_dividend_capacity_calculator(self):
        ...
        with col2:
            # Prudential gates - FIXED: Use safe access to config values
            ...
            try:
                ...
                capital_adequacy_gate = 15.0  # Default: 15% SASRA minimum
        ...
        with col3:
            # Dividend policy & simulation - FIXED: Use safe values
            ...
            **Policy Parameters:**
            ...
            # Calculate button
            if st.button("🚀 Calculate Dividend Capacity", 
                        type="primary", 
                        use_container_width=True,
                        key="calculate_dividend_btn"):
                with st.spinner("🔍 Analyzing surplus distribution and checking prudential gates..."):
                    # Create config dictionary with our safe values
                    dividend_config = {
                        'liquidity_gate': liquidity_gate / 100,
                        'par30_gate': par30_gate / 100,
                        'capital_adequacy_gate': capital_adequacy_gate / 100,
                        'max_recommendation': max_recommendation / 100
                    }
                    ...
                    result = self.dividend_calc.calculate_dividend_capacity(
                        ...
                        current_capital_adequacy=capital_adequacy / 100,
                        ...
                        config=dividend_config  # Pass our config dictionary instead
                    )
    ...
    def render_dividend_results(self, result):
        ...
        with col2:
            st.metric(
                ...
                delta=f"KES {result['dividend_amount'] - (result['previous_dividend_pct'] * result['total_shares'] / 100):+,.0f}" if result['previous_dividend_pct'] else None,
                help="Total dividend to be paid to members"
            )
            st.caption(f"Per share: KES {result['dividend_amount'] / (result['total_shares'] / 100):.2f}")
        ...
        with col4:
            st.metric(
                ...
                delta=f"KES {(result['available_for_distribution'] - result['dividend_amount']):+,.0f} remaining",
                help="Funds available after all provisions and reserves"
            )
            st.caption(f"Utilization: {result['dividend_amount']/result['available_for_distribution']*100:.1f}%")
        
        # Detailed policy gates analysis
        ...
        distribution_data = pd.DataFrame({
            ...
            'Type': ['Start', 'Reserve', 'Provision', 'Reserve', 'Intermediate', 'Dividend', 'End']
        })
        ...
        fig = go.Figure(go.Waterfall(
            ...
            text=[f"KES {abs(x):,.0f}" for x in distribution_data['Amount (KES)']],
        ...
        with col1:
            ...
            dividend_amounts = [rate * result['total_shares'] / 100 for rate in rate_scenarios]
                
            # Use safe max_recommendation value
            try:
                max_rate = self.config.dividend.max_recommendation * 100
            except AttributeError:
                max_rate = 7.0
                
            feasibility = ['Feasible' if amount <= result['available_for_distribution'] else 'Not Feasible' 
                            for amount in dividend_amounts]
                
            sensitivity_df = pd.DataFrame({
                'Dividend Rate (%)': rate_scenarios,
                'Dividend Amount (KES M)': [amount/1000000 for amount in dividend_amounts],
                'Feasibility': feasibility,
                'Policy Compliant': ['Yes' if rate <= max_rate else 'No' 
            ...
            fig = px.scatter(
                ...
                size_max=15,
                color_discrete_map={'Feasible': '#10b981', 'Not Feasible': '#ef4444'}
            )
            
            fig.add_hline(y=result['available_for_distribution']/1000000, 
                         line_dash="dash", 
                         line_color="gray",
    ...
    def render_prudential_gate_analysis(self):
        """Render detailed prudential gate analysis"""
        ...
        with col1:
            ...
            for gate in result['policy_gates']:
                if 'current_value' in gate and 'threshold' in gate:
                    ...
                    if gate['name'] == 'Liquidity Ratio':
                        score = min(100, (gate['current_value'] / gate['threshold']) * 100)
                    elif gate['name'] == 'PAR30 Ratio':
                        score = min(100, (gate['threshold'] / gate['current_value']) * 100)
            ...
            fig = go.Figure(data=go.Scatterpolar(
                r=gate_performance + [gate_performance[0]],  # Close the shape
    ...
    def render_historical_performance(self):
        """Render historical dividend performance analysis"""
        ...
        with col1:
            # Historical trend with multiple metrics - FIXED: Use title_font instead of titlefont
            fig = go.Figure()
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    title="Dividend Rate (%)",
                    title_font=dict(color='#10b981'),  # FIXED: title_font not titlefont
                    tickfont=dict(color='#10b981')
                ),
                yaxis2=dict(
                    title="Net Surplus (KES M)",
                    title_font=dict(color='#3b82f6'),  # FIXED: title_font not titlefont
                ...
                yaxis3=dict(
                    ...
                    title_font=dict(color='#f59e0b'),  # FIXED: title_font not titlefont
        ...
        with col2:
            ...
            correlation_data = historical_data[['Dividend_Rate', 'Net_Surplus_KES_M', 
                                               'Liquidity_Ratio', 'PAR30', 'Capital_Adequacy',
                                               'Dividend_Payout_Ratio']].corr()
            ...
            comparison_df['Year_Over_Year_Growth'] = comparison_df['Dividend_Rate'].pct_change() * 100
            comparison_df['Payout_Sustainability'] = comparison_df['Dividend_Payout_Ratio'].apply(
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Payout_Sustainability": st.column_config.TextColumn("Sustainability")
                }
            )
    
    def render_agm_preparation_recommendations(self):
        """Render AGM preparation and recommendations"""
        ...
        # Board voting simulation
        st.markdown("##### 🗳️ Board Voting Simulation")
        
        col1, col2, col3 = st.columns(3)
        
        with col1:
            board_size = st.number_input("Board Members Present", min_value=1, max_value=20, value=9)
        with col2:
            votes_for = st.slider("Votes in Favor", min_value=0, max_value=board_size, value=7)
        with col3:
            votes_against = board_size - votes_for
        ...
        if st.button("📋 Generate Dividend Recommendation Paper", use_container_width=True):
            st.success("✅ Dividend recommendation paper generated successfully!")
            st.info("Document includes: Financial analysis, Prudential gate assessment, Dividend calculation, Board recommendation")
        
        if st.button("📊 Generate Member Communication", use_container_width=True):
            st.success("✅ Member communication templates generated!")
            st.info("Includes: Dividend announcement, Payment schedule, Tax implications, Frequently asked questions")
    
    def render_dividend_policy_framework(self):
        """Render comprehensive dividend policy framework"""
        ...
        compliance_data = pd.DataFrame({
            'Policy Area': ['Statutory Reserve', 'Liquidity Gate', 'PAR30 Gate', 
                           'Capital Adequacy', 'Maximum Dividend', 'Board Discretion'],
            'Compliance Score': [100, 95, 98, 100, 100, 90],
            'Status': ['✅ Compliant', '✅ Compliant', '✅ Compliant', 
                      '✅ Compliant', '✅ Compliant', '⚠️ Review'],
            'Last Review': ['2024-01-15', '2024-01-20', '2024-01-20', 
                           '2024-01-15', '2024-01-10', '2023-12-15']
        })
        ...
        fig = px.bar(
            ...
            color_discrete_map={'✅ Compliant': '#10b981', '⚠️ Review': '#f59e0b'},
    ...
    def run(self):
        ...
        try:
            # Try to log with available method
            self.audit_logger.log_data_access(
                st.session_state.user,
                st.session_state.role,
            ...
            # Silently continue if logging fails
            print(f"Audit logging warning: {e}")
            pass
