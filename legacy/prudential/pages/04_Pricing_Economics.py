...
st.set_page_config(
    page_title="🏛️ Pricing Intelligence & Economics",
    page_icon="💳",
    layout="wide"
)
...
class PricingEconomicsPage:
    ...
    def render_current_pricing_intelligence(self):
        ...
        with col1:
            ...
            filtered_data['Rate_Gap_vs_Market'] = filtered_data['Current_Rate'] - filtered_data['Market_Average']
            filtered_data['Rate_Gap_vs_Risk_Adjusted'] = filtered_data['Current_Rate'] - filtered_data['Risk_Adjusted_Rate']
            
            fig = go.Figure(data=[
                go.Bar(name='vs Market', 
                       x=filtered_data['Product'], 
                       y=filtered_data['Rate_Gap_vs_Market'],
                       marker_color=['red' if x < 0 else 'green' for x in filtered_data['Rate_Gap_vs_Market']]),
                go.Bar(name='vs Risk Adjusted', 
                       x=filtered_data['Product'], 
                       y=filtered_data['Rate_Gap_vs_Risk_Adjusted'],
                       marker_color=['red' if x < 0 else 'green' for x in filtered_data['Rate_Gap_vs_Risk_Adjusted']])
            ])
        ...
        with col2:
            ...
            for _, product in filtered_data.iterrows():
                ...
                score = (
                    ...
                    ((product['Current_Rate'] - product['Risk_Adjusted_Rate']) / 5 * 20)
                )
                ...
                performance_metrics.append({
                    ...
                    'Rate Alignment': 'Optimal' if abs(product['Current_Rate'] - product['Risk_Adjusted_Rate']) < 0.5 
                                     else 'High' if product['Current_Rate'] > product['Risk_Adjusted_Rate'] 
                                     else 'Low',
            ...
            if sort_by == "Profit Margin":
                performance_df = performance_df.sort_values('Performance Score', ascending=False)
            elif sort_by == "Volume":
                performance_df = performance_df.merge(filtered_data[['Product', 'Volume_KES_M']], on='Product')
                performance_df = performance_df.sort_values('Volume_KES_M', ascending=False)
    ...
    def render_risk_based_pricing_optimizer(self):
        ...
        with col2:
            ...
            total_risk_premium = base_risk_premium + credit_adjustment + dti_adjustment + collateral_adjustment
            
            st.markdown("#### 🎯 Calculated Risk Premium")
            
            risk_premium_breakdown = pd.DataFrame({
                'Component': ['Base Risk', 'Credit Score', 'DTI Ratio', 'Collateral', 'Total'],
                'Premium (%)': [base_risk_premium, credit_adjustment, dti_adjustment, 
                               collateral_adjustment, total_risk_premium]
            })
        ...
        with col_calc2:
            if st.button("🚀 Calculate Optimal Pricing", 
                        type="primary", 
                        use_container_width=True,
                        key="calculate_pricing_btn"):
                with st.spinner("🔍 Calculating risk-based optimal pricing..."):
                    pricing_result = self.pricing_optimizer.calculate_comprehensive_pricing(
    ...
    def _get_risk_premium(self, risk_category: str) -> float:
        """Get base risk premium based on category"""
        premiums = {
            "Low": 1.5,
            "Medium": 3.0,
            "High": 6.0,
    ...
    def _get_collateral_adjustment(self, collateral: float) -> float:
        """Get collateral coverage adjustment"""
        if collateral >= 150:
            return -2.0
        elif collateral >= 120:
            return -1.0
    ...
    def render_pricing_results(self, result):
        ...
        with col2:
            ...
            st.caption(f"Margin: {(result['recommended_rate'] - result['break_even_rate']) * 100:.2f}%")
        ...
        with sensitivity_tab1:
            ...
            with col_sens1:
                ...
                rates_cof = [self.pricing_optimizer.calculate_rate_sensitivity(
                    base_rate=result['recommended_rate'],
                    cost_change=(cof/100 - result['cost_of_funds'])
                ) * 100 for cof in cof_range]
                
                fig = px.line(
                    x=cof_range,
                    y=rates_cof,
                    title="Sensitivity to Cost of Funds",
                    labels={'x': 'Cost of Funds (%)', 'y': 'Recommended Rate (%)'}
                )
                fig.add_vline(x=result['cost_of_funds']*100, line_dash="dash", line_color="red")
                st.plotly_chart(fig, use_container_width=True)
            
            with col_sens2:
                # Operating cost sensitivity
                opex_range = np.linspace(1.5, 3.5, 9)
                rates_opex = [self.pricing_optimizer.calculate_rate_sensitivity(
                    base_rate=result['recommended_rate'],
                    cost_change=(opex/100 - result['operating_cost'])
                ) * 100 for opex in opex_range]
                
                fig = px.line(
                    x=opex_range,
                    y=rates_opex,
                    title="Sensitivity to Operating Costs",
                    labels={'x': 'Operating Cost (%)', 'y': 'Recommended Rate (%)'}
                )
                fig.add_vline(x=result['operating_cost']*100, line_dash="dash", line_color="red")
                st.plotly_chart(fig, use_container_width=True)
        ...
        with sensitivity_tab1:
            ...
            with col_sens1:
                ...
                for cof in cof_range:
                    # Calculate new rate with changed cost of funds
                    new_rate = result['recommended_rate'] + (cof/100 - result['cost_of_funds'])
                ...
                fig.add_vline(x=result['cost_of_funds']*100, line_dash="dash", line_color="red")
                fig.add_annotation(x=result['cost_of_funds']*100, y=rates_cof[4], 
            ...
            with col_sens2:
                ...
                for opex in opex_range:
                    new_rate = result['recommended_rate'] + (opex/100 - result['operating_cost'])
                ...
                fig.add_vline(x=result['operating_cost']*100, line_dash="dash", line_color="red")
                fig.add_annotation(x=result['operating_cost']*100, y=rates_opex[4], 
        ...
        with sensitivity_tab2:
            ...
            with col_sens3:
                ...
                fig = px.line(
                    ...
                    labels={'x': 'Risk Premium Change (%)', 'y': 'Adjusted Rate (%)'}
                )
                fig.add_vline(x=0, line_dash="dash", line_color="red")
                fig.add_annotation(x=0, y=rates_risk[5], 
            ...
            with col_sens4:
                ...
                for score in credit_scores:
                    # Simulate credit score impact on rate
                    if score >= 750:
                        adjustment = -0.015
                    elif score >= 700:
                        adjustment = -0.005
                ...
                fig.add_vline(x=result['credit_score'], line_dash="dash", line_color="red")
                fig.add_annotation(x=result['credit_score'], y=rates_credit[5], 
            ...
            for category in risk_categories:
                # Calculate risk premium for each category
                ...
                adjusted_rate = result['base_rate'] + total_risk + result.get('strategic_adjustment', 0)
        ...
        with sensitivity_tab3:
            ...
            with col_sens5:
                ...
                for market_change in market_range:
                    ...
                    # Recalculate rate with new market position
                    if result['competitive_positioning'] == "Price Leader":
                        adjustment = max(new_market - 0.005 - result['recommended_rate'], 0)
                    elif result['competitive_positioning'] == "Premium Position":
                        adjustment = new_market + 0.005 - result['recommended_rate']
                    else:  # Market Match
                ...
                fig.add_vline(x=0, line_dash="dash", line_color="red")
                fig.add_annotation(x=0, y=rates_market[5], 
            ...
            with col_sens6:
                ...
                for position in positions:
                    if position == "Price Leader":
                        adjustment = max(result['market_rate'] - 0.005 - result['recommended_rate'], 0)
                    elif position == "Premium Position":
                        adjustment = result['market_rate'] + 0.005 - result['recommended_rate']
                    else:  # Market Match
                        if abs(result['recommended_rate'] - result['market_rate']) > 0.01:
                            adjustment = result['market_rate'] - result['recommended_rate']
                        else:
                            adjustment = 0
                ...
                fig = px.bar(
                    ...
                    labels={'x': 'Positioning Strategy', 'y': 'Adjusted Rate (%)'},
                    color=rates_position,
                    color_continuous_scale='RdYlGn_r'  # Red for higher rates, green for lower
                )
                
                # Add current position highlight
                current_idx = positions.index(result.get('competitive_positioning', 'Market Match'))
                fig.add_shape(
                    type="rect",
                    x0=current_idx-0.4, x1=current_idx+0.4,
                    y0=0, y1=max(rates_position) * 1.1,
                    line=dict(color="gold", width=2),
                    fillcolor="rgba(255, 215, 0, 0.1)"
                )
            ...
            competitiveness_score = 100 - abs(result['recommended_rate'] - result['market_rate']) * 1000
            ...
            with col_gauge2:
                ...
                gap_data = {
                    'Metric': ['Rate vs Market', 'Risk Premium', 'Strategic Adjustment', 'Total Gap'],
                    'Value': [
                    ...
                    'Status': [
                        '✅ Competitive' if abs(result['recommended_rate'] - result['market_rate']) <= 0.01 else '⚠️ Review',
                        '✅ Appropriate' if 0.01 <= result['risk_premium'] <= 0.05 else '⚠️ Review',
                        '✅ Optimal' if abs(result.get('strategic_adjustment', 0)) <= 0.005 else '⚠️ Adjust',
                        '✅ Profitable' if result['recommended_rate'] > result['break_even_rate'] else '❌ Loss'
                    ]
                }
            ...
            if market_gap > 0.02:
                recommendations.append("📉 **Consider rate reduction**: Rate is >2% above market")
            elif market_gap < -0.02:
                recommendations.append("📈 **Consider rate increase**: Rate is >2% below market")
            else:
                recommendations.append("✅ **Maintain current positioning**: Rate is within market range")
            
            # Risk premium recommendation
            if result['risk_premium'] > 0.05:
                recommendations.append("⚡ **Review risk premium**: Premium >5% may reduce competitiveness")
            elif result['risk_premium'] < 0.01:
                recommendations.append("⚠️ **Increase risk coverage**: Premium <1% may not cover risks")
            
            # Profitability check
            if result['recommended_rate'] < result['break_even_rate']:
                recommendations.append("🚨 **CRITICAL**: Rate below break-even - Immediate action required")
    ...
    def render_competitive_intelligence(self):
        ...
        competitors = ['SACCO A', 'SACCO B', 'Commercial Bank', 'Microfinance', 'Fintech', 'Market Average', 'Our SACCO']
        products = ['Personal Loan', 'Business Loan', 'Asset Finance', 'Emergency Loan', 'School Fees Loan']
        
        # Generate comprehensive competitor rates
        competitor_data = []
        for competitor in competitors:
            for product in products:
                # Base rates with variations
                base_rates = {
                    'Personal Loan': 14.0,
                    'Business Loan': 16.0,
                    'Asset Finance': 13.0,
                ...
                if competitor == 'Our SACCO':
                    ...
                    # Competitor-specific variations
                    variation = {
                ...
                competitor_data.append({
                    ...
                    'Product': product,
                    'Rate': rate,
                    'Market Position': 'Leader' if rate < base_rate else 'Follower',
                    'Risk Profile': 'Aggressive' if rate < base_rate - 1 else 'Conservative' if rate > base_rate + 1 else 'Neutral'
                })
        ...
        with col2:
            ...
            positioning_data = []
            for product in products:
                our_rate = competitor_df[(competitor_df['Competitor'] == 'Our SACCO') & 
                                        (competitor_df['Product'] == product)]['Rate'].iloc[0]
                market_avg = competitor_df[(competitor_df['Competitor'] == 'Market Average') & 
                                          (competitor_df['Product'] == product)]['Rate'].iloc[0]
                ...
                positioning_data.append({
                    ...
                    'Position': 'Leader' if our_rate < market_avg else 'Follower'
                })
        ...
        strategy_data['Competitive_Index'] = strategy_data.groupby('Product')['Rate'].rank(method='dense', ascending=True)
    ...
    def render_deposit_pricing_strategy(self):
        ...
        # Enhanced deposit products data
        deposit_products = pd.DataFrame({
            'Product': ['Savings Account', 'Fixed Deposit 30D', 'Fixed Deposit 90D', 
                       'Fixed Deposit 180D', 'Fixed Deposit 1Y', 'Youth Account', 
                       'Senior Citizen Account', 'Premium Savings'],
        ...
        with col2:
            ...
            deposit_products['Rate_Gap'] = deposit_products['Current_Rate'] - deposit_products['Market_Average']
            deposit_products['Growth_Potential'] = deposit_products['Growth_Rate'] * deposit_products['Stability_Score'] / 100
            ...
            fig.add_hline(y=deposit_products['Growth_Potential'].median(), 
                         line_dash="dash", line_color="gray")
            fig.add_vline(x=0, line_dash="dash", line_color="gray")
            
            fig.add_annotation(x=1, y=8, text="⭐ Stars", 
                             showarrow=False, font=dict(color="green", size=12))
            fig.add_annotation(x=-1, y=8, text="📈 Growth Focus", 
                             showarrow=False, font=dict(color="blue", size=12))
            fig.add_annotation(x=1, y=4, text="💰 Profit Focus", 
                             showarrow=False, font=dict(color="orange", size=12))
            fig.add_annotation(x=-1, y=4, text="🔍 Review Needed", 
        ...
        for _, product in deposit_products.iterrows():
            if product['Rate_Gap'] < -0.3:  # More than 0.3% below market
                action = f"Increase to {product['Market_Average']:.1f}% (+{abs(product['Rate_Gap']):.1f}%)"
                priority = "High"
            elif product['Rate_Gap'] > 0.3:  # More than 0.3% above market
        ...
        roadmap_phases = [
            {
                ...
                "kpis": ["NIM: 8.5%", "Pricing efficiency: 90%", "Market competitiveness: 5/5 products"],
                "budget": "KES 500K"
            },
            {
                ...
                "kpis": ["Risk-adjusted returns: +1%", "Customer satisfaction: 90%", "Market share: +2%"],
                "budget": "KES 1.2M"
            },
            {
                ...
                "kpis": ["Automated pricing: 80%", "Response time: <24 hours", "ROI on pricing: 15%"],
                "budget": "KES 2.5M"
            },
            {
                ...
                "kpis": ["Market leadership: 3 products", "Pricing innovation index: 85%", "Strategic alignment: 95%"],
                "budget": "KES 3.8M"
            }
        ]
        
        for phase in roadmap_phases:
            with st.expander(f"📅 {phase['phase']} - Budget: {phase['budget']}", expanded=True):
                col1, col2, col3, col4 = st.columns(4)
