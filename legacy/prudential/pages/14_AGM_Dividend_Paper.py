...
class AGMDividendPaperPage:
            ...
            with col1:
                ...
                strategy_data = {
                    ...
                    'Current': [
                        ...
                        f"{(1 - dividend_capacity.get('payout_ratio', 0))*100:.1f}%",
                        "92%",
                        "75-85%"
                    ],
                    'Target': [
    ...
    def _render_shareholder_analytics_dashboard(self, analysis):
        ...
        if dividend_allocations:
            ...
            with col1:
                ...
                segments = {
                    "Premium (>KES 50,000)": sum(1 for d in dividend_amounts if d > 50000),
                    "Gold (KES 20,000-50,000)": sum(1 for d in dividend_amounts if 20000 <= d <= 50000),
                    "Silver (KES 5,000-20,000)": sum(1 for d in dividend_amounts if 5000 <= d < 20000),
                    "Standard (<KES 5,000)": sum(1 for d in dividend_amounts if d < 5000)
                }
            ...
            with col2:
                ...
                for i, alloc in enumerate(top_allocations, 1):
                    with st.expander(f"🥇 #{i}: {alloc.member_name}"):
                        ...
                        with col_b:
                            st.metric("Dividend per Share", f"KES {alloc.net_dividend/alloc.share_count if alloc.share_count > 0 else 0:.3f}")
    ...
    def _render_compliance_documentation_hub(self, analysis):
        ...
        for module in training_modules:
            ...
            with col3:
                if st.button(f"🔄 SCHEDULE REFRESH", key=f"refresh_{module['module']}"):
                    st.info(f"Refresh scheduled for {module['next_refresh']}")
    
    def render_agm_dashboard(self):
        """PRESERVED: Original AGM dashboard functionality"""
        try:
            # Generate AGM report
