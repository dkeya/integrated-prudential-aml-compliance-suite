# pages/01_Overview.py - ENHANCED WITH ENTERPRISE FEATURES
...
from sacco_core.rbac import RBACManager, check_permission, get_current_role, get_current_username
...
# Page configuration
st.set_page_config(
    page_title="🏛️ Prudential Compliance Overview",
    page_icon="📊",
    layout="wide"
)
...
class OverviewPage:
        ...
        if not has_access:
            ...
            try:
                self.audit_logger.log_action(
                    ...
                    object_type="page",
                    object_id="01_Overview.py"
                )
            except:
                pass  # Silently continue if logging fails
            return False
        
        # Log successful page access - using available method
        try:
            ...
            try:
                # Fallback to log_action if log_data_access doesn't work
                self.audit_logger.log_action(
    ...
    def render_strategic_header(self):
        ...
        <div style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%);
            ...
            <div style="margin-top: 10px; font-size: 0.9rem; opacity: 0.8;">
                ...
                <strong>👤</strong> {st.session_state.get('role', 'User')}
            </div>
        </div>
    ...
    def render_portfolio_intelligence(self):
        ...
        with col1:
            # Enhanced portfolio growth with annotations
            fig = px.line(
                portfolio_data, 
                x='Date', 
                y='Total Portfolio (KES M)',
                title="🏦 Loan Portfolio Growth Trend",
                labels={'Total Portfolio (KES M)': 'Portfolio Value (KES M)', 'Date': ''},
                line_shape="spline"
            )
            
            # Add growth rate annotations
            for i in range(1, len(portfolio_data)):
                growth = ((portfolio_data.iloc[i]['Total Portfolio (KES M)'] - 
                          portfolio_data.iloc[i-1]['Total Portfolio (KES M)']) / 
                         portfolio_data.iloc[i-1]['Total Portfolio (KES M)'] * 100)
                
                if i % 3 == 0:  # Annotate every 3rd point
                    fig.add_annotation(
                        x=portfolio_data.iloc[i]['Date'],
                        y=portfolio_data.iloc[i]['Total Portfolio (KES M)'],
                        text=f"+{growth:.1f}%",
                        showarrow=True,
                        arrowhead=1,
                        font=dict(size=10)
        ...
        with col2:
            ...
            fig.add_hline(y=product_metrics['Yield (%)'].mean(), 
                         line_dash="dash", line_color="gray")
            fig.add_vline(x=product_metrics['PAR (%)'].mean(), 
                         line_dash="dash", line_color="gray")
            
            fig.add_annotation(x=2, y=18, text="⭐ Stars", 
                             showarrow=False, font=dict(color="green", size=12))
            fig.add_annotation(x=2, y=11, text="📊 Cash Cows", 
                             showarrow=False, font=dict(color="blue", size=12))
            fig.add_annotation(x=7, y=18, text="⚠️ Question Marks", 
                             showarrow=False, font=dict(color="orange", size=12))
            fig.add_annotation(x=7, y=11, text="🔍 Review Needed", 
            ...
            fig.add_hline(
                y=self.config.limits.par30_warning_min * 100, 
                line_dash="dot", 
                line_color="orange",
                annotation_text="Warning Level (4%)",
    ...
    def render_risk_analytics(self):
        ...
        with col1:
            ...
            risk_data = pd.DataFrame({
                'Risk Category': ['Credit Risk', 'Liquidity Risk', 'Concentration Risk', 
                ...
                'Trend': ['Improving', 'Stable', 'Worsening', 'Improving', 'Stable', 'Worsening']
            })
            ...
            for i, row in risk_data.iterrows():
                fig.add_shape(
                    type="line",
                    x0=i-0.4, x1=i+0.4,
                    y0=row['Target Level'], y1=row['Target Level'],
            ...
            employer_data = pd.DataFrame({
                'Employer': ['Govt Agency A', 'Manufacturing B', 'School C', 'Hospital D', 'Others'],
        ...
        with col2:
            ...
            categories = ['Credit Quality', 'Liquidity', 'Concentration', 
                         'Profitability', 'Growth', 'Compliance']
            ...
            risk_metrics = pd.DataFrame({
                'Metric': ['PAR 30+ Days', 'PAR 90+ Days', 'NPL Ratio', 'Provision Coverage',
                          'Liquidity Ratio', 'Capital Adequacy', 'ROA', 'Cost to Income'],
                'Current': ['4.2%', '1.8%', '2.5%', '85%', '18.5%', '22.1%', '1.8%', '62%'],
                'Target': ['<4.0%', '<1.5%', '<2.0%', '>80%', '>20%', '>20%', '>2.0%', '<60%'],
                'Status': ['⚠️ Watch', '✅ Good', '⚠️ Watch', '✅ Good', 
                          '⚠️ Watch', '✅ Good', '⚠️ Watch', '⚠️ Watch']
            })
            ...
            st.dataframe(
                ...
                column_config={
                    "Status": st.column_config.TextColumn(
                        "Status",
                        help="Compliance status indicator"
                    )
                }
            )
    ...
    def render_compliance_monitoring(self):
        ...
        with col1:
            ...
            compliance_data = pd.DataFrame({
                'Area': ['Capital Adequacy', 'Asset Quality', 'Management', 'Earnings', 
                        'Liquidity', 'Sensitivity', 'Governance'],
                'Score': [95, 88, 92, 85, 78, 90, 82],
                'Status': ['✅ Compliant', '✅ Compliant', '✅ Compliant', '⚠️ Watch', 
                          '⚠️ Watch', '✅ Compliant', '⚠️ Watch']
            })
            ...
            deadlines = pd.DataFrame({
                'Requirement': ['SASRA Q4 Returns', 'Annual Audit', 'Board Risk Report', 
                               'Member AGM', 'CBK Reporting', 'Tax Compliance'],
                'Due Date': ['2024-01-31', '2024-02-28', '2024-03-15', 
                            '2024-03-31', '2024-04-30', '2024-05-15'],
                'Days Left': [15, 43, 58, 74, 104, 119],
                'Status': ['In Progress', 'Scheduled', 'Planned', 'Planned', 'Upcoming', 'Upcoming']
            })
        ...
        with col2:
            ...
            alerts_data = {
                'Date': ['2024-01-15 14:30', '2024-01-14 09:15', '2024-01-13 16:45', 
                        '2024-01-12 11:20', '2024-01-11 10:00'],
                'Alert Type': ['PAR Warning', 'Concentration', 'Liquidity', 
                              'Employer Limit', 'Governance'],
                ...
                'Owner': ['Risk Manager', 'CEO', 'Treasurer', 'Risk Committee', 'Secretary']
            }
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "Owner": st.column_config.SelectboxColumn(
                        "Owner",
                        help="Action owner",
                        options=["Risk Manager", "CEO", "Treasurer", "Risk Committee", "Compliance Officer"]
                    )
                }
            )
    ...
    def render_strategic_initiatives(self):
        ...
        for initiative in initiatives:
            with st.expander(f"{initiative['name']} - {initiative['progress']}% complete", expanded=True):
                ...
                st.progress(initiative['progress']/100, text=f"Implementation Status")
                st.caption(initiative['description'])
                
                col_a, col_b = st.columns(2)
                with col_a:
                    if st.button(f"📋 View Details", key=f"details_{initiative['name']}"):
                        st.info(f"Detailed plan loaded for {initiative['name']}")
                with col_b:
                    if st.button(f"🚀 Update Progress", key=f"update_{initiative['name']}"):
    ...
    def render_performance_frameworks(self):
        ...
        implementation_data = pd.DataFrame({
            'Framework': ['CAMELS', 'BCG Matrix', 'Risk-Adjusted Return', 'SWOT', 'PDCA', 'Balanced Scorecard'],
