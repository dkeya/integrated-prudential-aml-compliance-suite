...
st.set_page_config(
    page_title="🏛️ Credit Risk Intelligence & PAR Analysis",
    page_icon="📈",
    layout="wide"
)
...
class CreditRiskPage:
    ...
    def render_par_intelligence_dashboard(self):
        ...
        with col1:
            ...
            for i, (col, name, color) in enumerate(zip(bucket_columns, bucket_names, colors)):
            ...
            for metric in metrics:
                ...
                with col_b:
                    if metric['current'] > metric['threshold']:
                        st.markdown(f"<div style='color: #dc2626; font-size: 0.8rem;'>🔴 {threshold_status}</div>", 
                                  unsafe_allow_html=True)
                    else:
                        st.markdown(f"<div style='color: #10b981; font-size: 0.8rem;'>🟢 {threshold_status}</div>", 
                                  unsafe_allow_html=True)
            
            # Risk heatmap by delinquency stage
            st.markdown("##### 🔥 Delinquency Migration Heatmap")
            
            # Simulate migration matrix
    ...
    def render_concentration_management(self):
        ...
        with col1:
            # Concentration analysis with limits
        ...
        for plan in action_plans:
            with st.expander(f"{plan['priority']} Priority: {plan['risk']} - {plan['status']}", expanded=True):
                ...
                with col4:
                    ...
                    st.markdown(f"**Status:** <span style='color:{status_color};'>{plan['status']}</span>", 
                              unsafe_allow_html=True)
    ...
    def run(self):
        ...
        # Log dashboard completion using existing AuditLogger API
        try:
            ...
            # Fail silently if logging is not available
            pass
