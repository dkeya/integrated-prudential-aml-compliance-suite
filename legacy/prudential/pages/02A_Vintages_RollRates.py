...
st.set_page_config(
    page_title="🏛️ Vintage Intelligence & Roll Rate Analytics",
    page_icon="📊",
    layout="wide"
)
...
class VintagesRollRatesPage:
    ...
    def render_cohort_performance_analytics(self):
        ...
        with col1:
            ...
            p = np.poly1d(z)
            fig.add_trace(go.Scatter(
                x=vintage_data['avg_credit_score'].sort_values(),
                y=p(vintage_data['avg_credit_score'].sort_values()),
                mode='lines',
                name='Trend Line',
        ...
        with col2:
            ...
            for cohort in cohorts_to_show:
                ...
                survival_rate = [100 - (base_default * (month/12) * np.random.uniform(0.8, 1.2)) 
                               for month in months]
            ...
            performance_matrix = vintage_data[['cohort', 'total_disbursed', 'cumulative_default_rate', 
                                             'avg_credit_score', 'recovery_rate']].copy()
        ...
        for insight in insights:
            with st.expander(f"{insight['priority']} Priority: {insight['insight']}", expanded=False):
                ...
                with col4:
                    ...
                    st.markdown(f"**Priority:** <span style='color:{priority_color};'>{insight['priority']}</span>", 
                              unsafe_allow_html=True)
